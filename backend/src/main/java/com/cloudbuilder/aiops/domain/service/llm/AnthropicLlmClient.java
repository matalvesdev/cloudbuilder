...
/**
 * Anthropic LLM client — calls the Messages API.
 *
 * TIER 0 reference: https://docs.anthropic.com/en/api/messages
 * Requires configuration: cloudbuilder.ai.llm.anthropic.api-key, cloudbuilder.ai.llm.anthropic.model (default: claude-sonnet-4-20250514)
 *
 * Activated when `cloudbuilder.ai.llm.provider` is set to "anthropic".
 * Falls back gracefully to rule-based on errors.
 */
@Component
@ConditionalOnProperty(name = "cloudbuilder.ai.llm.provider", havingValue = "anthropic")
public class AnthropicLlmClient implements LlmClient {
    ...
    public AnthropicLlmClient(
            @Value("${cloudbuilder.ai.llm.anthropic.api-key:}") String apiKey,
            @Value("${cloudbuilder.ai.llm.anthropic.model:claude-sonnet-4-20250514}") String model,
        ...
        if (!this.enabled) {
            log.warn("Anthropic LLM client configured but no API key provided — falling back to rule-based");
        }
    }
    ...
    public String chat(String systemPrompt, String userMessage, Map<String, Object> context) {
        ...
        try {
            var body = buildMessagesBody(systemPrompt, userMessage);
            var response = post(body);
            return extractContent(response);
        } catch (Exception e) {
            log.warn("Anthropic chat failed, using fallback: {}", e.getMessage());
    ...
    public String chatFallback(String systemPrompt, String userMessage, Map<String, Object> context, Exception ex) {
        log.warn("Circuit breaker OPEN for Anthropic chat, using rule-based fallback: {}", ex.getMessage());
        return fallback.chat(systemPrompt, userMessage, context);
    }
    ...
    public String analyzeMetric(String metricName, List<Double> recentValues, double threshold) {
        ...
        try {
            ...
            var body = buildMessagesBody("Você é um SRE especialista em análise de métricas.", prompt);
            var response = post(body);
            return extractContent(response);
        } catch (Exception e) {
            log.warn("Anthropic metric analysis failed, using fallback: {}", e.getMessage());
    ...
    public String analyzeMetricFallback(String metricName, List<Double> recentValues, double threshold, Exception ex) {
        log.warn("Circuit breaker OPEN for Anthropic analyzeMetric, using rule-based fallback: {}", ex.getMessage());
        return fallback.analyzeMetric(metricName, recentValues, threshold);
    }
    ...
    public String generateRca(String incidentTitle, String incidentDescription,
        ...
        try {
            ...
            log.warn("Anthropic RCA generation failed, using fallback: {}", e.getMessage());
            return fallback.generateRca(incidentTitle, incidentDescription, severity, relatedMetrics, relatedLogs);
        }
    }
    ...
    public String generateRcaFallback(String incidentTitle, String incidentDescription,
        ...
        log.warn("Circuit breaker OPEN for Anthropic generateRca, using rule-based fallback: {}", ex.getMessage());
        return fallback.generateRca(incidentTitle, incidentDescription, severity, relatedMetrics, relatedLogs);
    }

    private ObjectNode buildMessagesBody(String systemPrompt, String userMessage) {
    ...
    private String extractContent(ResponseEntity<String> response) {
        ...
        try {
            ...
            if (content.isArray() && content.size() > 0) {
                return content.get(0).path("text").asText("Resposta não disponível.");
            }
            return root.path("content").asText("Resposta não disponível.");
        } catch (Exception e) {

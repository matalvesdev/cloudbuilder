...
describe("canvasStore", () => {
  ...
  describe("updateNodeProperties", () => {
    it("atualiza propriedades do nó", () => {
      ...
      useCanvasStore
        .getState()
        .updateNodeProperties(nodeId, { cidr: "10.0.0.0/16" });
    ...
    it("faz merge com propriedades existentes", () => {
      ...
      useCanvasStore
        .getState()
        .updateNodeProperties(nodeId, { region: "us-east-1" });
      expect(useCanvasStore.getState().nodes[0].data?.properties).toEqual({
        cidr: "10.0.0.0/16",
        region: "us-east-1",
      });
  ...
  describe("updateNodeLabel", () => {
    it("altera o label do nó", () => {
      ...
      useCanvasStore
        .getState()
        .updateNodeLabel(nodeId, "Minha VPC Customizada");
      expect(useCanvasStore.getState().nodes[0].data?.label).toBe(
        "Minha VPC Customizada",
      );
    });
  });

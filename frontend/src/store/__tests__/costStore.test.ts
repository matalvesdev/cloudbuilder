...
describe('costStore', () => {
  ...
  describe('applyOptimization', () => {
    it('marca otimização como aplicada', () => {
      useCostStore.setState({
        optimizations: [{
          id: 'opt-1', resourceName: 'test', resourceType: 'compute',
          provider: 'aws', currentCost: 100, estimatedCost: 50,
          savings: 50, savingsPercent: 50,
          suggestion: 'Reduza', severity: 'high', applied: false,
        }],
      })
    ...
    it('não altera otimizações com id diferente', () => {
      useCostStore.setState({
        optimizations: [
          { id: 'opt-1', resourceName: 'a', resourceType: 'compute',
            provider: 'aws', currentCost: 100, estimatedCost: 50,
            savings: 50, savingsPercent: 50,
            suggestion: 'x', severity: 'high', applied: false },
          { id: 'opt-2', resourceName: 'b', resourceType: 'compute',
            provider: 'aws', currentCost: 200, estimatedCost: 100,
            savings: 100, savingsPercent: 50,
            suggestion: 'y', severity: 'high', applied: false },
        ],
      })
      ...
      useCostStore.setState({
        optimizations: [
          ...
          { id: 'opt-2', resourceName: 'b', resourceType: 'compute',
            provider: 'aws', currentCost: 200, estimatedCost: 100,
            savings: 100, savingsPercent: 50,
            suggestion: 'y', severity: 'high', applied: true },
        ],
      })
    ...
    it('retorna 0 se todas aplicadas', () => {
      useCostStore.setState({
        optimizations: [{
          ...
          provider: 'aws', currentCost: 100, estimatedCost: 50,
          savings: 50, savingsPercent: 50,
          suggestion: 'x', severity: 'high', applied: true,
        }],
      })
  ...
  describe('fetchCostData (API integration)', () => {
    ...
    it('atualiza costSummary quando API retorna dados', async () => {
      ...
      vi.spyOn(mockDashboardApi.dashboardApi, 'getCostOverview').mockResolvedValue({
        totalCost: 5000,
        forecast: 5500,
    ...
    it('mantém mock data quando API falha', async () => {
      const mockDashboardApi = await import('@/api/dashboardApi')
      vi.spyOn(mockDashboardApi.dashboardApi, 'getCostOverview').mockRejectedValue(new Error('Network error'))
    ...
    it('atualiza loading state durante e após fetch', async () => {
      const mockDashboardApi = await import('@/api/dashboardApi')
      vi.spyOn(mockDashboardApi.dashboardApi, 'getCostOverview').mockImplementation(
        () => new Promise((resolve) => setTimeout(() => resolve({
          totalCost: 5000, forecast: 5500,
    ...
    it('usa environment do localStorage', async () => {
      ...
      const mockDashboardApi = await import('@/api/dashboardApi')
      const spy = vi.spyOn(mockDashboardApi.dashboardApi, 'getCostOverview').mockResolvedValue({
        totalCost: 100, forecast: 110,
        periodStart: '', periodEnd: '',
        topServices: [], budgets: [],
      })

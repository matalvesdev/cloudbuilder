...
describe("activityStore", () => {
  describe("addEvent", () => {
    ...
    it("respeita o limite maxEvents", () => {
      ...
      for (let i = 0; i < 5; i++) {
        useActivityStore
          .getState()
          .addEvent(makeEvent({ title: `Evento ${i}` }));
      }
  ...
  describe("getByType", () => {
    it("filtra eventos por tipo", () => {
      ...
      useActivityStore
        .getState()
        .addEvent(makeEvent({ type: "deploy_success" }));
      useActivityStore.getState().addEvent(makeEvent({ type: "design_save" }));
    ...
    it("retorna vazio se nenhum evento do tipo", () => {
      expect(
        useActivityStore.getState().getByType("drift_detected"),
      ).toHaveLength(0);
    });
  });
  ...
  describe("fetchActivityEvents (API integration)", () => {
    ...
    it("usa environment do localStorage", async () => {
      ...
      const mockDashboardApi = await import("@/api/dashboardApi");
      const spy = vi
        .spyOn(mockDashboardApi.dashboardApi, "getObserveDashboard")
        .mockResolvedValue({
          totalServices: 1,
          degradedCount: 0,
          downCount: 0,

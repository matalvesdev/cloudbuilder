...
interface IncidentState {
  ...
  addFixHistory: (
    entry: Omit<FixHistoryEntry, "id" | "appliedAt">,
  ) => FixHistoryEntry;
...
export const useIncidentStore = create<IncidentState>()((set, get) => ({
  ...
  fetchIncidents: async (environmentId: string) => {
    ...
    try {
      const data = await aiopsApi.listIncidents();
  ...
  analyzeIncident: async (incidentId: string) => {
    try {
      return await (aiopsApi as any).analyzeIncident(incidentId);
    } catch {
      return null;
    }
  },
  ...
  resolveIncident: async (incidentId: string) => {
    try {
      return await (aiopsApi as any).resolveIncident(incidentId);
    } catch {
      return null;
    }
  },

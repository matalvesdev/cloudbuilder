...
beforeEach(() => {
  useCostStore.setState({
    costSummary: { totalMonthly: 0, byProvider: {} as Record<string, number>, byService: {}, currency: 'USD' },
    costHistory: [],
    optimizations: [],
    selectedMonth: '',
    loading: false,
...
describe('costStore — property-based', () => {
  ...
  it('error string via setState', () => {
    fc.assert(
      fc.property(fc.option(fc.string({ minLength: 1, maxLength: 100 })), (error) => {
    ...
    fc.assert(
      fc.property(
        ...
        (history) => {
          ...
          expect(useCostStore.getState().costHistory).toHaveLength(history.length)
        },
      ),
      { numRuns: 200 },
    )
    ...
    fc.assert(
      fc.property(
        ...
        (opts) => {
          ...
          expect(useCostStore.getState().optimizations).toHaveLength(opts.length)
        },
      ),
      { numRuns: 200 },
    )
  ...
  it('applyOptimization marks matching id as applied', () => {
    ...
    useCostStore.setState({
      optimizations: [
        { id, resourceName: 'i-123', resourceType: 'ec2', provider: 'aws', currentCost: 200, estimatedCost: 100, savings: 100, savingsPercent: 50, suggestion: 'downsize', severity: 'high', applied: false },
      ],
    })
  ...
  it('totalSavings sums unapplied optimization savings', () => {
    useCostStore.setState({
      optimizations: [
        { id: '1', resourceName: 'a', resourceType: 'ec2', provider: 'aws', currentCost: 200, estimatedCost: 100, savings: 100, savingsPercent: 50, suggestion: 'a', severity: 'high', applied: false },
        { id: '2', resourceName: 'b', resourceType: 's3', provider: 'aws', currentCost: 300, estimatedCost: 100, savings: 200, savingsPercent: 66, suggestion: 'b', severity: 'medium', applied: true },
        { id: '3', resourceName: 'c', resourceType: 'rds', provider: 'aws', currentCost: 100, estimatedCost: 50, savings: 50, savingsPercent: 50, suggestion: 'c', severity: 'low', applied: false },
      ],
    })
    ...
    fc.assert(
      fc.property(
        ...
        (alerts) => {
          ...
          expect(useCostStore.getState().budgetAlerts).toHaveLength(alerts.length)
        },
      ),
      { numRuns: 200 },
    )

...
export const useTeamStore = create<TeamState>()((set, get) => ({
  ...
  loadTeams: async (orgId) => {
    ...
    try {
      ...
      set({
        error: (err as Error).message || "Erro ao carregar times",
        loading: false,
      });
    }
  },
  ...
  loadSquads: async (orgId, workspaceId) => {
    ...
    try {
      ...
      set({
        error: (err as Error).message || "Erro ao carregar squads",
        loading: false,
      });
    }
  },
  ...
  loadMembersByTeam: async (orgId, teamId) => {
    ...
    try {
      ...
      set({
        error: (err as Error).message || "Erro ao carregar membros",
        loading: false,
      });
    }
  },
  ...
  createSquadAction: async (orgId, workspaceId, name, description, leadId) => {
    set({ error: null });
    try {
      const squad = await createSquad(orgId, {
        workspaceId,
        name,
        description,
        leadId,
  ...
  updateSquadAction: async (orgId, id, name, description, leadId) => {
    ...
    try {
      ...
      set((s) => ({
        squads: s.squads.map((sq) => (sq.id === id ? squad : sq)),
      }));
      return squad;
    } catch (err) {

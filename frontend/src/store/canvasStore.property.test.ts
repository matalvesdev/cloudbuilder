...
const makeComponentData = (overrides?: Partial<CanvasNodeData> & { id?: string }) => ({
      ...
      fc.property(
        ...
        (count) => {
          ...
          for (let i = 0; i < count; i++) {
            useCanvasStore.getState().addNode(makeComponentData(), { x: i * 10, y: i * 10 })
          }
  ...
  it('every node has a unique UUID id', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 50 }), (count) => {
        ...
        for (let i = 0; i < count; i++) {
          useCanvasStore.getState().addNode(makeComponentData(), { x: 0, y: 0 })
        }
  ...
  it('node type matches provider', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (provider) => {
          useCanvasStore.setState({ nodes: [] })
          useCanvasStore.getState().addNode(makeComponentData({ provider }), { x: 0, y: 0 })
  ...
  it('removing a node reduces count by exactly 1', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 20 }), (count) => {
        ...
        for (let i = 0; i < count; i++) {
          useCanvasStore.getState().addNode(makeComponentData({ id: `node-${i}` }), { x: i * 50, y: 0 })
        }
  ...
  it('removing a node removes all connected edges', () => {
    fc.assert(
      fc.property(fc.integer({ min: 2, max: 10 }), (count) => {
        useCanvasStore.setState({ nodes: [], edges: [] })
        for (let i = 0; i < count; i++) {
          useCanvasStore.getState().addNode(makeComponentData({ id: `n-${i}` }), { x: i * 100, y: 0 })
        }
        // Connect first node to all others
        const nodes = useCanvasStore.getState().nodes
        for (let i = 1; i < count; i++) {
          useCanvasStore.getState().addEdgeWithType(nodes[0].id, nodes[i].id, 'default')
        }
  ...
  it('adding an edge always increases edge count by 1', () => {
    fc.assert(
      fc.property(
        ...
        (extraEdges, edgeType) => {
          useCanvasStore.setState({ nodes: [], edges: [] })
          useCanvasStore.getState().addNode(makeComponentData({ id: 'a' }), { x: 0, y: 0 })
          useCanvasStore.getState().addNode(makeComponentData({ id: 'b' }), { x: 100, y: 0 })
          const nodes = useCanvasStore.getState().nodes

          // Add some edges
          for (let i = 0; i < extraEdges; i++) {
            useCanvasStore.getState().addEdgeWithType(nodes[0].id, nodes[1].id, edgeType)
          }

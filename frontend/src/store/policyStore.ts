...
function detectResourceType(
  resourceType: string,
): "storage" | "compute" | "network" | "database" | "security" | null {
...
function hasTag(node: Node<CanvasNodeData>, tag: string): boolean {
  ...
  if (props.tags && typeof props.tags === "object" && tag in props.tags)
    return true;
  if (Array.isArray(props.tags))
    return props.tags.some((t: any) =>
      typeof t === "string" ? t === tag : t.key === tag || t.Key === tag,
    );
  return false;
}
...
const BUILT_IN_POLICIES: Policy[] = [
  {
    ...
    description:
      "Todos os recursos de armazenamento devem ter criptografia habilitada",
    category: "security",
    severity: "critical",
    resourceTypes: [
      "s3_bucket",
  ...
  },
  {
    ...
    description:
      "Security groups não devem permitir acesso 0.0.0.0/0 na porta 22 (SSH)",
    category: "security",
    severity: "critical",
...
function isInstanceTypeResource(resourceType: string): boolean {
  return ["ec2_instance", "rds_instance", "elasticache", "node_group"].includes(
    resourceType,
  );
}
...
): { key: InstanceProperty; value: string } | null {
  const props = node.data.properties;
  if (!props) return null;
  if (props.instanceClass)
    return { key: "instanceClass", value: props.instanceClass as string };
  if (props.instanceType)
    return { key: "instanceType", value: props.instanceType as string };
  if (props.nodeType)
    return { key: "nodeType", value: props.nodeType as string };
  return null;
}
  ...
  checkPolicies: () => {
    ...
    for (const policy of BUILT_IN_POLICIES) {
      for (const node of canvasNodes) {
        ...
        let violated = false;
        let description = "";
        const autoFixAvailable = policy.autoFixable;
        ...
        if (isStorageResource(resourceType) && policy.id === "encryption") {
          if (resourceType === "s3_bucket" && !props.encryption) {
            ...
            !props.sseSpecification?.enabled &&
            !props.sseEnabled
          ) {
            violated = true;
        ...
        if (isSecurityGroup(resourceType) && policy.id === "iam-principle") {
          const rules = props.inboundRules as
            | Array<{
                port?: number;
                cidr?: string;
                fromPort?: number;
          ...
          if (rules) {
            const openSsh = rules.some(
              (r) =>
                (r.port === 22 || r.fromPort === 22) && r.cidr === "0.0.0.0/0",
            );
            if (openSsh) {
              violated = true;
        ...
        if (
          isInstanceTypeResource(resourceType) &&
          policy.id === "cost-control"
        ) {

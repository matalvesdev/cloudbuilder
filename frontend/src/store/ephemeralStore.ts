...
function estimateCost(ttlHours: number, resourceSize: ResourceSize): number {
  const sizeCost =
    resourceSize === "small" ? 0 : resourceSize === "medium" ? 0.5 : 1.2;
...
export const useEphemeralStore = create<EphemeralState>((set, get) => ({
  ...
  createEphemeral: async (req) => {
    ...
    try {
      ...
      set((s) => ({
        environments: [local, ...s.environments],
        creating: false,
      }));
    }
  },
    ...
    set((s) => ({
      environments: s.environments.map((e) => {
        ...
        return {
          ...
          ttl_hours: newTtl,
          expiresAt: new Date(
            new Date(e.expiresAt).getTime() + extraHours * 3600000,
          ).toISOString(),
  ...
  getActiveCount: () => {
    return get().environments.filter(
      (e) => e.status === "active" || e.status === "creating",
    ).length;
  },

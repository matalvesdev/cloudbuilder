...
function makeFlag(
  overrides: Partial<FeatureFlagDTO> &
    Pick<FeatureFlagDTO, "flagKey" | "enabled">,
): FeatureFlagDTO {
  return {
  ...
  it("random flag states produce deterministic results", () => {
    fc.assert(
      fc.property(fc.boolean(), fc.boolean(), fc.boolean(), (e1, e2, e3) => {
        ...
        expect(useUiStore.getState().isEnabled("feature.what-if-cost")).toBe(
          e3,
        );
      }),
      { numRuns: 500 },

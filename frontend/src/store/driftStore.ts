...
function toDriftResource(item: DriftItemDTO, envId: string): DriftResource {
  ...
  const driftType =
    (item.driftType as DriftResource["driftType"]) === "ADDED"
      ? "ADDED"
...
function toDriftReport(dto: DriftReportDTO): DriftReport {
  const resources =
    dto.drifts?.map((d) => toDriftResource(d, dto.environmentId)) ?? [];
  return {
    id: dto.id,
    ...
    status: (dto.status === "RESOLVED"
      ...
      : dto.status === "IGNORED"
        ? "IGNORED"
        : "DETECTED") as DriftReport["status"],
  };
}
...
export const useDriftStore = create<DriftState>()((set, get) => ({
  ...
  loadDriftReport: async () => {
    ...
    try {
      ...
      set({
        error: err instanceof Error ? err.message : "Falha ao detectar drift",
      });
    } finally {
      set({ loading: false });
    }
  ...
  remediateDrift: (resourceId) => {
    set((state) => ({
      reports: state.reports.map((r) => ({
        ...
        resources: r.resources.map((res) =>
          res.id === resourceId
            ? { ...res, status: "REMEDIATED" as const }
            : res,
        ),
      })),
    }));

...
class WebSocketClient {
  ...
  disconnect(): void {
    ...
    if (this.ws) {
      // Null out ALL callbacks to prevent handlers firing after disconnect
      // (critical when StrictMode unmounts before the WebSocket finishes opening)
      this.ws.onopen = null;
      this.ws.onerror = null;
      this.ws.onclose = null;
      this.ws.onmessage = null;
      // Only call close() if the socket finished opening — calling close()
      // on a CONNECTING socket logs "WebSocket is closed before the connection is established"
  ...
  bridgeToEventBus(allowedPrefixes?: string[]): void {
    ...
    this.onMessage((message) => {
      ...
      eventBus.publish(
        eventType,
        message.payload as DomainEvents[typeof eventType],
      );
    });
  }

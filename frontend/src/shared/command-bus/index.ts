...
export interface Command<TPayload = Record<string, unknown>, TResult = unknown> {
  type: string
  payload: TPayload
  meta?: CommandMeta
}
...
class CommandBus {
  ...
  register<TPayload extends Record<string, unknown>, TResult>(
    ...
    if (this.handlers.has(type)) {
      console.warn(`[CommandBus] Overwriting handler for command type: "${type}"`)
    }
  ...
  async dispatch<TPayload extends Record<string, unknown>, TResult>(
    ...
    const baseHandler = (cmd: Command): Promise<CommandResult> => handler(cmd as Command<Record<string, unknown>, unknown>)
    const execute = this.middleware.reduceRight<((cmd: Command) => Promise<CommandResult>)>(
      (next, mw) => (cmd: Command) => mw(cmd, next),
      baseHandler
    )
    ...
    const promise = execute(command).then((result) => result as CommandResult<TResult>).finally(() => {

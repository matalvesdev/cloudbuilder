...
const createCanvasHandler: CommandHandler<{ name: string; description?: string }> = async (cmd) => {
  const data = await api.post('/canvases', cmd.payload)
  return { success: true, data }
}

const updateCanvasHandler: CommandHandler<{ id: string; name?: string; description?: string }> = async (cmd) => {
...
const addNodeHandler: CommandHandler<{ canvasId: string; type: string; positionX: number; positionY: number; properties?: string }> = async (cmd) => {
  const { canvasId, ...body } = cmd.payload
  const data = await api.post(`/canvases/${canvasId}/nodes`, body)
  return { success: true, data }
}

const removeNodeHandler: CommandHandler<{ canvasId: string; nodeId: string }> = async (cmd) => {
  const { canvasId, nodeId } = cmd.payload
  await api.delete(`/canvases/${canvasId}/nodes/${nodeId}`)
  return { success: true }
}

const addEdgeHandler: CommandHandler<{ canvasId: string; source: string; target: string; edgeType?: string }> = async (cmd) => {
  const { canvasId, ...body } = cmd.payload
  const data = await api.post(`/canvases/${canvasId}/edges`, body)
  return { success: true, data }
}

const removeEdgeHandler: CommandHandler<{ canvasId: string; edgeId: string }> = async (cmd) => {
  const { canvasId, edgeId } = cmd.payload
  await api.delete(`/canvases/${canvasId}/edges/${edgeId}`)
  return { success: true }
}

const validateCanvasHandler: CommandHandler<{ canvasId: string }> = async (cmd) => {
...
const generateCodeHandler: CommandHandler<{ canvasId: string; engine?: string }> = async (cmd) => {
  const engine = cmd.payload.engine ?? 'terraform'
  const data = await api.post(`/canvases/${cmd.payload.canvasId}/generate?engine=${engine}`)
  return { success: true, data }
}

const syncEnvironmentHandler: CommandHandler<{ environmentId: string; stateJson?: string }> = async (cmd) => {
  const { environmentId, ...body } = cmd.payload
  const data = await api.post(`/environments/${environmentId}/sync`, body)
  return { success: true, data }
}

const resolveDriftHandler: CommandHandler<{ environmentId: string; reportId: string; resolvedBy?: string }> = async (cmd) => {
  const { environmentId, reportId, resolvedBy } = cmd.payload
  const data = await api.post(`/environments/${environmentId}/drift/resolve/${reportId}`, { resolvedBy })
  return { success: true, data }
}

/* ─── Deployment Commands ────────────────────────────────────── */

const deployHandler: CommandHandler<{ environmentId: string; version?: string }> = async (cmd) => {
  const data = await api.post(`/environments/${cmd.payload.environmentId}/deploy`, cmd.payload)
  return { success: true, data }
}

/* ─── Cost Commands ──────────────────────────────────────────── */

const createBudgetHandler: CommandHandler<{ environmentId: string; name: string; limit: number }> = async (cmd) => {
...
const createCredentialHandler: CommandHandler<{ name: string; type: string; value: string }> = async (cmd) => {
...
const createComplianceRuleHandler: CommandHandler<{ rule: Record<string, unknown> }> = async (cmd) => {
  const tenantId = localStorage.getItem('cloudbuilder-active-tenant-id') || 'default'
  const data = await api.post(`/compliance/rules/${tenantId}`, cmd.payload.rule)
  return { success: true, data }
}

const deleteComplianceRuleHandler: CommandHandler<{ id: string }> = async (cmd) => {

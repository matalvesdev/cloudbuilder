...
const OPA_POLICIES = [
  {
    ...
    description:
      "Políticas de otimização de custos — limites de gasto, instâncias reservadas, spot",
    icon: FileCode,
  ...
  },
  {
    ...
    description:
      "Políticas de governança — tagging, regiões permitidas, compliance organizacional",
    icon: FileCode,
    ...
    >
      <div
        className={cn(
          ...
          status.reachable ? "bg-green-500" : "bg-amber-500",
        )}
      />
    ...
    true
          ...
          <div className="flex items-center justify-between pt-1">
            <p className="text-[10px] text-slate-400">
              Edite o arquivo{" "}
              <span className="font-mono">{policy.id}.rego</span> no diretório{" "}
              <span className="font-mono">
                opa/policies/compliance/cloudbuilder/
              </span>
            </p>
...
const REGO_SYNTAX = [
  ...
  {
    token: "<rule> { <conditions> }",
    desc: "Regra Rego — true se todas condições forem satisfeitas",
  },
  {
    token: "input.<field>",
    desc: "Acessa campo do input enviado na requisição",
  },
  ...
  {
    token: "every <var> in <collection> { ... }",
    desc: "Iteração com verificação universal",
  },
];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">
            Visualização das políticas Policy-as-Code gerenciadas pelo OPA
            sidecar (ADR-020)
          </p>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-ice-blue/30 border border-ice-blue/50 text-sm text-brand-navy">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Os arquivos <span className="font-mono">.rego</span> são carregados
            do diretório <span className="font-mono">opa/policies/</span> no
            servidor. Para modificar, edite os arquivos Rego diretamente no
            repositório da plataforma ou use o endpoint{" "}
            <span className="font-mono">POST /api/v1/compliance/rules</span>{" "}
            para criar regras de conformidade.
          </p>
        </div>
      </div>

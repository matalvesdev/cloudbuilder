...
function getActionStyle(action: string) {
  return ACTION_COLORS[action.toUpperCase()] || { bg: 'bg-slate-50', text: 'text-slate-600', label: action }
}
...
function ExportButton({ format, params }: { format: 'csv' | 'json'; params: AuditQueryParams }) {
...
export function AuditTimelineView() {
  ...
  const buildParams = useCallback((): AuditQueryParams => ({
    userId: filterUserId || undefined,
    action: filterAction || undefined,
    resourceType: filterResourceType || undefined,
    startDate: filterStartDate || undefined,
    endDate: filterEndDate || undefined,
  }), [filterUserId, filterAction, filterResourceType, filterStartDate, filterEndDate])
  ...
  const hasActiveFilters = filterUserId || filterAction || filterResourceType || filterStartDate || filterEndDate
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-brand-navy font-display">Timeline de Auditoria</h2>
          <p className="text-sm text-slate-400">Eventos registrados em ordem cronológica</p>
        </div>
      ...
      <div className="flex items-center gap-2">
        <Button
          ...
          className={cn('gap-1.5', showFilters && 'border-brand-navy bg-brand-navy/5')}
        >
        ...
        {hasActiveFilters && (
          <Button variant="ghost" size="sm" onClick={handleClearFilters} className="gap-1.5 text-slate-400">
      ...
      {showFilters && (
        <div className="bg-white rounded-2xl card-shadow border border-slate-100 p-5 space-y-4">
          <div className="grid grid-cols-5 gap-4">
            <div className="flex flex-col gap-1">
              <label className="text-[11px] font-bold tracking-widest text-slate-400 uppercase">Usuário</label>
              <Input
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[11px] font-bold tracking-widest text-slate-400 uppercase">Ação</label>
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[11px] font-bold tracking-widest text-slate-400 uppercase">Tipo de Recurso</label>
              <Input
            ...
            <DateInput label="Data Início" value={filterStartDate} onChange={setFilterStartDate} />
            <DateInput label="Data Fim" value={filterEndDate} onChange={setFilterEndDate} />
          </div>
          <div className="flex justify-end">
            <Button size="sm" onClick={handleSearch} className="gap-1.5 bg-brand-navy text-white hover:bg-brand-navy/90">
      ...
      {eventsError && !loadingEvents && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <Button variant="outline" size="sm" className="mt-4" onClick={() => fetchEvents(undefined, buildParams())}>
            Tentar novamente
          </Button>
        </div>
      )}
      ...
      {!loadingEvents && !eventsError && events.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-base font-semibold text-brand-navy">Nenhum evento de auditoria encontrado</p>
          <p className="text-sm text-slate-400 mt-1">
            {hasActiveFilters ? 'Tente ajustar os filtros para ampliar a busca.' : 'Nenhuma ação foi registrada até o momento.'}
          </p>
        </div>
      )}
        ...
        <div className="bg-white rounded-3xl card-shadow border border-slate-100 overflow-hidden">
          <div className="relative">
            {events.map((evt, idx) => {
              ...
              return (
                <div key={evt.id} className="flex gap-4 px-6 py-4 hover:bg-slate-50/50 transition-colors">
                  {/* Timeline line + dot */}
                  <div className="flex flex-col items-center shrink-0 pt-1">
                    <div className={cn(
                      'w-2.5 h-2.5 rounded-full ring-2 ring-white',
                      actionStyle.text.replace('text-', 'bg-').replace('-700', '-500')
                    )} />
                  ...
                  <div className="flex-1 min-w-0 pb-4">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={cn('px-2 py-0.5 rounded text-[11px] font-semibold', actionStyle.bg, actionStyle.text)}>
                        {actionStyle.label}
                      </span>
                    ...
                    <div className="mt-2 space-y-1">
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="font-medium text-brand-navy">{evt.userId}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-slate-600">
                        <FileText className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                        <span>
                          <span className="font-mono text-xs bg-slate-100 px-1.5 py-0.5 rounded">{evt.resourceType}</span>
                          <span className="mx-1 text-slate-300">/</span>
                          <span className="font-mono text-xs">{evt.resourceId}</span>
                        </span>
                      </div>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/50">
            <p className="text-xs text-slate-400">
              Página <span className="font-medium text-slate-600">{currentPage + 1}</span>
            </p>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"

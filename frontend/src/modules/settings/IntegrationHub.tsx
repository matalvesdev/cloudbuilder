...
const CATEGORIES: Category[] = [
  {
    ...
    description: "Repositórios e controle de versão",
  },
  {
    id: "cloud",
    ...
    description: "Provedores de nuvem IaaS",
  },
  {
    id: "kubernetes",
    label: "Kubernetes",
    ...
    description: "Clusters Kubernetes gerenciados",
  },
  {
    id: "cicd",
    label: "CI/CD",
  ...
  },
  {
    ...
    description: "Bancos de dados e data services",
  },
  {
    id: "notifications",
    label: "Notificações",
  ...
  },
  {
    ...
    description: "Provedores de identidade SSO",
  },
  {
    id: "secrets",
    label: "Secrets",
  ...
  },
  {
    ...
    description: "Provedores de IA/LLM",
  },
  {
    id: "custom",
    label: "Custom",
...
const WIZARD_STEPS: WizardStep[] = [
  {
    ...
    description: "Escolha o serviço para conectar",
  },
  {
    id: "authenticate",
    title: "Autenticar",
  ...
  },
  {
    id: "configure",
    title: "Configurar",
    description: "Ajuste as opções de integração",
  },
...
export function IntegrationHub() {
  ...
  const handleWizardComplete = async () => {
    ...
    try {
      const cat = CATEGORIES.find((c) =>
        c.providers.includes(selectedProvider),
      );
      await integrationApi.create({
        name: PROVIDER_NAMES[selectedProvider] || selectedProvider,
        providerId: selectedProvider,
        category: cat?.id || "custom",
      });
      await fetchData();
      showSuccess(
        `${PROVIDER_NAMES[selectedProvider] || selectedProvider} conectado com sucesso`,
      );
    } catch (err) {
  ...
  const getConnectedCount = (categoryId: string) =>
    integrations.filter(
      (i) => i.category === categoryId && i.status === "CONNECTED",
    ).length;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        ...
        <p className="text-xs text-slate-400 mb-5">
          Selecione uma categoria para ver os provedores disponíveis
        </p>
      ...
      {activeCategory &&
        (() => {
          const catProviders =
            CATEGORIES.find((c) => c.id === activeCategory)?.providers || [];
          const filtered = catProviders.filter((p) => {
            // Search filter
            if (
              providerSearch &&
              !(PROVIDER_NAMES[p] || p)
            ...
            if (statusFilter !== "all") {
              const isConnected = integrations.some(
                (i) => i.providerId === p && i.status === "CONNECTED",
              );
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {[
                    ...
                    <button
                      ...
                      className={cn(
                        "px-2 h-8 rounded-lg text-[10px] font-semibold border transition-all",
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    className={cn(
                      "px-2 h-8 rounded-lg text-[10px] font-semibold border transition-all flex items-center gap-1",
                      sortBy === "name"
                        ? "bg-brand-navy text-white border-brand-navy"
                        : "bg-white text-slate-500 border-slate-200 hover:border-slate-300",
                    )}
                ...
                {paginatedProviders.length === 0 ? (
                  ...
                  paginatedProviders.map((providerId) => {
                    ...
                    const health = connected
                      ? HEALTH_CONFIG[connected.healthStatus] ||
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          {connected ? (
                            <>
                              <button
                                onClick={() => handleHealthCheck(connected.id)}
                                disabled={healthChecking === connected.id}
                              ...
                              <button
                                onClick={() => setShowDetail(connected.id)}
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
                  <p className="text-[10px] text-slate-400" aria-live="polite">
                    Mostrando {(providerPage - 1) * PROVIDERS_PER_PAGE + 1}–
                    {Math.min(
                      providerPage * PROVIDERS_PER_PAGE,
                      filtered.length,
                    )}{" "}
                  ...
                  >
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                      (page) => (
                        <button
                          ...
                          aria-current={
                            page === providerPage ? "page" : undefined
                          }
                          className={cn(
                    ...
                    <button
                      onClick={() =>
                        setProviderPage((p) => Math.min(totalPages, p + 1))
                      }
      ...
      {integrations.length > 0 && !activeCategory && (
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          <h5 className="text-sm font-bold text-brand-navy mb-4">
            Integrações Ativas
          </h5>
          <div className="space-y-2">
            {integrations.map((int) => {
              const statusCfg =
                STATUS_CONFIG[int.status] || STATUS_CONFIG.DISCONNECTED;
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <StatusIcon
                      className={cn("w-4 h-4", statusCfg.color.split(" ")[0])}
                    />
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400">
                        {PROVIDER_NAMES[int.providerId] || int.providerId} •{" "}
                        {int.category}
                      </p>
                    </div>
                  </div>
          ...
          >
            <div className="p-6 border-b border-slate-100">
              <h3 className="text-lg font-bold text-brand-navy">
                Conectar {PROVIDER_NAMES[selectedProvider]}
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
              ...
              {wizardStep === 0 && (
                <div className="space-y-3">
                  <p className="text-xs text-slate-500">
                    ...
                    <strong>{PROVIDER_NAMES[selectedProvider]}</strong>
                  </p>
                  <p className="text-xs text-slate-400">
                    Este provedor será conectado ao CloudBuilder.
                  </p>
                </div>
              )}
              {wizardStep === 1 && (
              ...
              {wizardStep === 2 && (
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      type="text"
                      defaultValue={PROVIDER_NAMES[selectedProvider]}
                      className="w-full h-10 px-3 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-lime"
                    />
                  </div>
                </div>
              ...
              {wizardStep === 3 && (
                <div className="text-center py-4">
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Clique em "Conectar" para finalizar
                  </p>
                </div>
              )}
            </div>
            <div className="p-6 pt-0 flex items-center justify-end gap-2">
              ...
              {wizardStep < WIZARD_STEPS.length - 1 ? (
                ...
                <button
                  ...
                  disabled={connecting === selectedProvider}
      ...
      {showDetail &&
        (() => {
          ...
          const statusCfg =
            STATUS_CONFIG[int.status] || STATUS_CONFIG.DISCONNECTED;
          const health =
            HEALTH_CONFIG[int.healthStatus] || HEALTH_CONFIG.UNKNOWN;
          return (
            <div
              className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm"

...
const ROLE_OPTIONS: {
...
}[] = [
  {
    value: "admin",
    label: "Admin",
    desc: "Pode gerenciar membros e configurações",
    icon: Shield,
  },
  {
    value: "member",
    label: "Membro",
    desc: "Pode criar e editar recursos",
    icon: ShieldCheck,
  },
  {
    value: "viewer",
      ...
      >
        <div className="p-6 border-b border-slate-100">
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Crie um novo projeto para isolar designs, credenciais e ambientes
          </p>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      ...
      >
        <div className="p-6 border-b border-slate-100">
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Envie um convite para participar do projeto
          </p>
        </div>
        <div className="p-6 space-y-4">
          <div>
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        >
          <Building2
            className={cn(
              "w-5 h-5",
              isActive ? "text-brand-lime" : "text-brand-navy",
            )}
          />
        </div>
        {isActive && (
...
function MembersList({ projectId }: { projectId: string }) {
  const { members, getProjectMembers, removeMember, updateMemberRole } =
    useTenantStore();
...
export function MultiTenantSettings() {
  const { projects, activeProjectId, switchProject, deleteProject } =
    useTenantStore();
  ...
  const handleDeleteProject = (projectId: string) => {
    if (
      window.confirm(
        "Tem certeza que deseja excluir este projeto? Todos os dados associados serão perdidos.",
      )
    ) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Gerencie projetos e times com isolamento completo
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {filteredProjects.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-xs text-slate-400 mb-4 max-w-sm">
                Crie projetos para isolar ambientes, credenciais e designs de
                diferentes times ou clientes.
              </p>
              <button
                onClick={() => setShowCreateDialog(true)}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1.5 text-[10px] text-slate-400">
                    <span>
                      {activeProject.memberCount} membro
                      {activeProject.memberCount !== 1 ? "s" : ""}
                    </span>
                    <span>
                      {activeProject.resourceCount} recurso
                      {activeProject.resourceCount !== 1 ? "s" : ""}
                    </span>
                    <span>
                      Criado em{" "}
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="bg-amber-50 rounded-xl p-4 text-center border border-amber-200">
                <p className="text-2xl font-bold text-amber-700">
                  {activeProject.id.substring(0, 8)}
                </p>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-card">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">
                  Este projeto possui isolamento completo de recursos
                </p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              {[
                {
                  label: "Designs",
                  desc: "Canvas e diagramas isolados por projeto",
                  active: true,
                },
                {
                  label: "Credenciais",
                  desc: "Credenciais de nuvem não compartilhadas",
                  active: true,
                },
                {
                  label: "Ambientes",
                  desc: "Ambientes de deploy exclusivos do projeto",
                  active: true,
                },
              ].map((item) => (
                <div

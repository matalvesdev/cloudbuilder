...
function hasMinRole(userRoles: string[], minRole: string): boolean {
  const userMax = Math.max(
    ...userRoles.map((r) => ROLE_HIERARCHY[r] ?? -1),
  );
  return userMax >= (ROLE_HIERARCHY[minRole] ?? 0);
}
...
const sidebarSections: SidebarSection[] = [
  { id: "profile", label: "Meu Perfil", icon: User, minRole: "viewer" },
  {
    id: "organization",
    label: "Organização",
    icon: Building2,
  ...
  { id: "workspaces", label: "Workspaces", icon: Box, minRole: "admin" },
  { id: "teams", label: "Times", icon: Users, minRole: "admin" },
  { id: "members", label: "Usuários", icon: Users, minRole: "admin" },
  { id: "permissions", label: "Permissões", icon: Shield, minRole: "admin" },
  { id: "credentials", label: "Cloud Providers", icon: Cloud, minRole: "editor" },
  {
    id: "git-providers",
    label: "Git Providers",
    icon: GitBranch,
  ...
  { id: "integrations", label: "Integrações", icon: Link, minRole: "editor" },
  { id: "security", label: "Segurança", icon: ShieldCheck, minRole: "editor" },
  { id: "billing", label: "Billing", icon: CreditCard, minRole: "admin" },
  { id: "notifications", label: "Notificações", icon: Bell, minRole: "viewer" },
  { id: "audit", label: "Auditoria", icon: FileText, minRole: "admin" },
  { id: "api-tokens", label: "API Tokens", icon: Key, minRole: "viewer" },
  { id: "ssh-keys", label: "SSH Keys", icon: Fingerprint, minRole: "viewer" },
  { id: "system", label: "Preferências", icon: Palette, minRole: "viewer" },
  { id: "ai-settings", label: "IA", icon: Brain, minRole: "admin" },
];
...
function CredentialFormModal({
  ...
  const validate = () => {
    ...
    if (form.secret && form.secret.length < 10)
      e.secret = "Mínimo 10 caracteres";
    setErrors(e);
      ...
      >
        <div className="p-6 border-b border-slate-100">
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Configure as credenciais de acesso ao provedor de nuvem
          </p>
        </div>
        <div className="p-6 space-y-4">
          <div>
            ...
            <input
              ...
              className={cn(
                "w-full h-10 px-3 rounded-xl border text-sm focus:ring-2 focus:ring-brand-navy/20 focus:border-brand-navy bg-white transition-all",
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setErrors({ ...errors, keyId: "" });
              }}
              className={cn(
                "w-full h-10 px-3 rounded-xl border text-sm font-mono focus:ring-2 focus:ring-brand-navy/20 focus:border-brand-navy bg-white transition-all",
                errors.keyId ? "border-red-300" : "border-slate-200",
              )}
              placeholder={
                form.provider === "aws" ? "AKIAIOSFODNN7EXAMPLE" : ""
              }
            />
            {errors.keyId && (
              <p className="text-[10px] text-red-500 mt-1">{errors.keyId}</p>
            )}
          </div>
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">
            ...
            <div className="relative">
              <input
                ...
                className={cn(
                  "w-full h-10 px-3 pr-10 rounded-xl border text-sm font-mono focus:ring-2 focus:ring-brand-navy/20 focus:border-brand-navy bg-white transition-all",
                  errors.secret ? "border-red-300" : "border-slate-200",
                )}
                placeholder={
                  editId ? "•••••••• (deixar vazio para manter)" : ""
                }
              />
              <button
            ...
            {errors.secret && (
              <p className="text-[10px] text-red-500 mt-1">{errors.secret}</p>
            )}
          </div>
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">
...
function ProfileSection() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showApiError(err, "Erro ao salvar");
    } finally {
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        ...
        <div className="space-y-4">
          ...
          <button
            onClick={handleSave}
            disabled={
              !profileName.trim() || profileName === user?.name || profileSaving
            }
            className="inline-flex items-center gap-1.5 px-5 h-9 rounded-full text-xs font-bold bg-brand-navy text-white hover:bg-[#0D1B2A] transition-all disabled:opacity-50"
          >
            {profileSaving ? (
...
function SecuritySection() {
  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        <h4 className="text-sm font-bold text-brand-navy mb-4 flex items-center gap-2">
          <ShieldCheck className="w-4 h-4" /> Autenticação
        </h4>
        <div className="space-y-4">
          {[
            {
              label: "MFA (Autenticação Multifator)",
              desc: "Adicione uma camada extra de segurança com TOTP",
              enabled: false,
              icon: Shield,
            },
            {
              label: "SSO (Single Sign-On)",
              desc: "Login com Google, GitHub ou provedor corporativo",
              enabled: false,
              icon: Link,
            },
            {
              label: "Device Trust",
              desc: "Apenas dispositivos aprovados podem acessar",
              enabled: false,
              icon: Fingerprint,
            },
          ].map((item) => (
            ...
            >
              ...
              <button
                className={cn(
                  ...
                  item.enabled ? "bg-brand-lime" : "bg-slate-200",
                )}
              >
                <span
                  className={cn(
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        <h4 className="text-sm font-bold text-brand-navy mb-4 flex items-center gap-2">
          <Key className="w-4 h-4" /> Sessões
        </h4>
        <p className="text-xs text-slate-400 mb-3">
          Sessões ativas em seus dispositivos
        </p>
        ...
        {[
          {
            ...
            desc: "Receber notificações no email cadastrado",
          },
          {
            key: "deployComplete" as const,
            label: "Deploy concluído",
            desc: "Quando um deploy for concluído com sucesso",
          },
          {
            key: "deployFailed" as const,
            label: "Falha no deploy",
            desc: "Quando um deploy falhar",
          },
          {
          ...
          {
            key: "driftDetected" as const,
            label: "Drift detectado",
            desc: "Divergência entre estado desejado e real",
          },
        ].map(({ key, label, desc }) => (
          <div
            key={key}
...
function APITokensSection() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showApiError(err, "Erro ao criar token");
    } finally {
      setCreating(false);
    }
  };
      ...
      {createdToken && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4">
          ...
          <div className="flex items-center gap-2">
            <code className="flex-1 px-3 py-2 bg-white rounded-lg border border-green-200 text-xs font-mono text-green-800 break-all">
              {createdToken}
            </code>
            <button
              onClick={() => {
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Tokens para acesso programático à API
            </p>
          </div>
          <button
          ...
          <div className="mb-4 p-4 bg-slate-50 rounded-xl border border-slate-200">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 h-9 px-3 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-lime"
              />
              <button
...
function SSHKeysSection() {
  ...
  const handleAdd = async () => {
    ...
    setAdding(true);
    try {
      const result = await settingsApi.addSshKey(
        keyName.trim(),
        publicKey.trim(),
      );
      ...
      showApiError(err, "Erro ao adicionar chave");
    } finally {
      setAdding(false);
    }
  };
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h4 className="text-sm font-bold text-brand-navy flex items-center gap-2">
            <Fingerprint className="w-4 h-4" /> SSH Keys
          </h4>
          <p className="text-xs text-slate-400 mt-0.5">
            Chaves públicas para operações Git e acesso a servidores
          </p>
        </div>
        <button
        ...
        <div className="mb-4 p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
          <div>
            ...
            <input
              ...
              className="w-full h-9 px-3 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-lime"
            />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">
            ...
            <textarea
              ...
              className="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs font-mono bg-white focus:outline-none focus:ring-2 focus:ring-brand-lime resize-none"
            />
          </div>
          <button
            onClick={handleAdd}
            disabled={adding || !keyName.trim() || !publicKey.trim()}
            className="px-4 h-9 rounded-full text-xs font-bold bg-brand-navy text-white hover:bg-[#0D1B2A] disabled:opacity-50 transition-all"
          >
            {adding ? (
              <Loader2 className="w-3.5 h-3.5 animate-spin" />
...
function IntegrationsSection() {
  ...
  const getConnectedProviders = (category: string) => {
    return integrations
      .filter((i) => i.category === category && i.status === "CONNECTED")
      .map((i) => i.providerId);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div
            key={s.label}
            className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm text-center"
          >
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        ...
        <p className="text-xs text-slate-400 mb-5">
          Conecte provedores, serviços e ferramentas ao CloudBuilder
        </p>
            ...
            return (
              ...
              >
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">
                  {connected.length > 0
                    ? `${connected.length} conectado(s)`
                    : `${cat.providers.length} provedores`}
                </p>
              </button>
            );
          })}
            ...
            {categories
              ...
              ?.providers.map((providerId) => {
                const connected = integrations.find(
                  (i) =>
                    i.providerId === providerId && i.status === "CONNECTED",
                );
                return (
                  <div
                    key={providerId}
...
function AISettingsSection() {
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
      <h4 className="text-sm font-bold text-brand-navy mb-4 flex items-center gap-2">
        <Brain className="w-4 h-4" /> Configurações de IA
      </h4>
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-semibold text-slate-600 mb-1.5">
  ...
  const visibleSections = useMemo(
    () =>
      sidebarSections.filter((s) =>
        hasMinRole(userRoles, s.minRole ?? "viewer"),
      ),
    [userRoles],
  );
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-56 bg-white border-r border-slate-200 flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
          {visibleSections.map((section) => {
            const badge =
              section.id === "credentials" && credentials.length > 0
                ? credentials.length
                : section.id === "environments" && environments.length > 0
                  ? environments.length
            ...
            return (
              <button
                key={section.id}
                onClick={() => setTab(section.id as SettingsTab)}
                className={cn(
              ...
              >
                ...
                {badge !== null && badge !== undefined && badge > 0 && (
                  <span
                    className={cn(
                      "text-[9px] px-1.5 py-0.5 rounded-full font-bold min-w-[18px] text-center",
                      tab === section.id
          ...
          <div className="p-3 border-t border-slate-100">
            ...
            <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(getConfiguredCount() / 3) * 100}%` }}
              />
            </div>
          </div>
        )}
      ...
      <div className="flex-1 overflow-y-auto p-7">
        <div className="mb-6">
          <h2 className="text-lg font-bold text-brand-navy">
            {sectionTitle[tab] || "Configurações"}
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">
            {sectionDescription[tab] ||
                ...
                {[
                  {
                    ...
                    color: "amber",
                    count: credentials.filter((c) => c.provider === "aws")
                      .length,
                  },
                  {
                    ...
                    color: "blue",
                    count: credentials.filter((c) => c.provider === "azure")
                      .length,
                  },
                  {
                    id: "gcp" as Provider,
                    ...
                    color: "green",
                    count: credentials.filter((c) => c.provider === "gcp")
                      .length,
                  },
                ].map((p) => (
                  <button
                    ...
                    className={cn(
                      "p-4 rounded-xl border-2 text-left transition-all hover:shadow-md",
                      p.count > 0
                        ? "border-brand-lime bg-green-50/50"
                        : "border-slate-200 bg-white hover:border-slate-300",
                    )}
                  >
                    <div className="flex items-center justify-between mb-2">
            ...
            {credentials.length === 0 ? (
              <div className="bg-white border border-slate-200 rounded-xl p-8 shadow-sm text-center">
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Clique em um provedor acima para adicionar
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() =>
                            setDeleteConfirm({ type: "cred", id: cred.id })
                          }
        ...
        {tab === "billing" && (
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            ...
            <p className="text-xs text-slate-400">
              Gerencie plano, faturas e métodos de pagamento
            </p>
          </div>
        )}
        {tab === "audit" && (
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            ...
            <p className="text-xs text-slate-400">
              Log de todas as ações realizadas na plataforma
            </p>
          </div>
        )}
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-end gap-2">
                <button
                  onClick={() => setDeleteConfirm(null)}
                  className="px-4 h-9 rounded-full text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="px-5 h-9 rounded-full text-xs font-bold bg-red-600 text-white hover:bg-red-700"
                >
                  Excluir
                </button>
              </div>

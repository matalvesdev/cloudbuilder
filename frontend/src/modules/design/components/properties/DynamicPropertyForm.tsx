...
export function DynamicPropertyForm({ schema, values, onChange, errors }: DynamicPropertyFormProps) {
  return (
...
function FieldRenderer({ field, value, onChange, error }: FieldRendererProps) {
  switch (field.type) {
    ...
    case 'tags':
      return <TagsField field={field} value={value} onChange={onChange} error={error} />
    case 'cidr':
      return <CidrField field={field} value={value} onChange={onChange} error={error} />
    default:
      return null
  }
}

function LabelWrapper({ field, error, children }: { field: PropertyField; error?: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1">
      ...
      {field.description && !error && (
        <p className="text-[10px] text-slate-400 leading-tight">{field.description}</p>
      )}
  ...
  return (
    <LabelWrapper field={field} error={error}>
      <Input
        ...
        className={cn('mt-1 h-9 text-xs bg-slate-50 border-slate-200 rounded-lg focus:border-primary focus:ring-1 focus:ring-primary', error && 'border-red-500')}
  ...
  return (
    <LabelWrapper field={field} error={error}>
      <div ref={ref} className="relative mt-1">
        ...
        >
          <span className={cn(value ? 'font-mono text-slate-700' : 'text-slate-400')}>{displayValue}</span>
            ...
            {field.options.map((option) => (
              <button
                ...
                className={cn(
                  ...
                  option.value === value && 'bg-ice-blue text-brand-navy font-semibold'
                )}
              >
                {option.label}
              </button>
      ...
      >
        <div
          className={cn(
            ...
            boolValue ? 'bg-green-500 border-green-500 text-white' : 'border-slate-300 bg-white'
          )}
        >
          {boolValue && (
    ...
    <LabelWrapper field={field} error={error}>
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-1.5">
          {tags.map((tag, i) => (
            <Badge key={i} variant="secondary" className="text-[10px] px-1.5 py-0.5 rounded bg-ice-blue text-brand-navy border-0">
              {tag}
            </Badge>
          ))}
        </div>

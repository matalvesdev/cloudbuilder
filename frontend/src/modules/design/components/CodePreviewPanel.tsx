...
function highlightLine(line: string): React.ReactNode {
  ...
  if (trimmed.startsWith('resource "') || trimmed.startsWith('variable "') || trimmed.startsWith('output "') || trimmed.startsWith('provider "')) {
    const match = trimmed.match(/^(resource|variable|output|provider)\s+"([^"]+)"\s+"([^"]+)"/)
    if (match) {
      const [, keyword, type, name] = match
      const before = line.slice(0, line.indexOf(keyword))
      const after = line.slice(line.indexOf('"', line.indexOf(name) + name.length + 2) + 1)
      return <>{before}<span className="text-blue-500 font-semibold">{keyword}</span> <span className="text-green-600">"{type}"</span> <span className="text-purple-500">"{name}"</span>{after}</>
    }
    const match2 = trimmed.match(/^(resource|variable|output|provider)\s+"([^"]+)"/)
    if (match2) {
      const [, keyword, name] = match2
      const before = line.slice(0, line.indexOf(keyword))
      const after = line.slice(line.indexOf('"', line.indexOf(name) + name.length + 2) + 1)
      return <>{before}<span className="text-blue-500 font-semibold">{keyword}</span> <span className="text-green-600">"{name}"</span>{after}</>
    }
  }
  ...
  if (eqMatch) {
    ...
    if (val.startsWith('"')) {
      return <>{before}<span className="text-slate-700 font-medium">{eqMatch[1]}</span> = <span className="text-green-600">{val}</span></>
    }
    if (/^\d/.test(val)) {
      return <>{before}<span className="text-slate-700 font-medium">{eqMatch[1]}</span> = <span className="text-amber-600">{val}</span></>
    }
    if (val.startsWith('aws_') || val.startsWith('azurerm_') || val.startsWith('google_')) {
      return <>{before}<span className="text-slate-700 font-medium">{eqMatch[1]}</span> = <span className="text-red-500">{val}</span></>
    }
    return <>{before}<span className="text-slate-700 font-medium">{eqMatch[1]}</span> = <span className="text-slate-500">{val}</span></>
  }
  // Highlight provider blocks
  const provMatch = trimmed.match(/^provider\s+"([^"]+)"/)
  if (provMatch) {
    const before = line.slice(0, line.indexOf('provider'))
    return <>{before}<span className="text-blue-500 font-semibold">provider</span> <span className="text-green-600">"{provMatch[1]}"</span></>
  }
  return <>{line}</>
}

export function CodePreviewPanel({ expanded, onToggle }: { expanded: boolean; onToggle?: () => void }) {
        ...
        <div className="flex items-center gap-4 h-full">
          {codeTabs.length > 0 ? (
            codeTabs.map((tab) => (
              <button
                key={tab.id}
                onClick={(e) => { e.stopPropagation(); if (expanded) setActiveTab(tab.id) }}
                className={cn(
            ...
            <span className="font-mono text-xs text-slate-400">Nenhum recurso no canvas</span>
          )}
        </div>
            ...
            <pre className="space-y-0">
              {activeCode.content.split('\n').map((line, i) => (
                <div key={i} className="flex">
                  <span className="w-8 select-none text-right mr-4 text-slate-300 text-xs">{i + 1}</span>

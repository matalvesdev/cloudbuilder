...
function AwsNode(props: NodeProps<CanvasNode>) {
  const { data, selected } = props
  return (
    <div className={`rounded-xl border-2 bg-white card-shadow w-64 ${selected ? 'border-[#FF9900] ring-2 ring-brand-lime/40' : 'border-slate-100'}`}>
      <CanvasNodeToolbar {...props} />
      <Handle type="target" position={Position.Top} className="w-3 h-3 !bg-[#FF9900]" />
      <div className="bg-[#FF9900] text-white px-3 py-2 rounded-t-xl flex items-center gap-2">
      ...
      <div className="px-3 py-2 text-xs text-slate-600 bg-white rounded-b-xl">
        <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-amber-50 border border-amber-100 text-amber-700 text-[10px] font-bold">
          {data.resourceType}
        </div>
      ...
      <Handle type="source" position={Position.Bottom} className="w-3 h-3 !bg-[#FF9900]" />

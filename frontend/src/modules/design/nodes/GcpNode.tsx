...
function GcpNode(props: NodeProps<CanvasNode>) {
  ...
  return (
    <div className={`rounded-xl border-2 bg-white card-shadow w-64 ${selected ? 'border-[#4285F4] ring-2 ring-brand-lime/40' : 'border-slate-100'}`}>
      <CanvasNodeToolbar {...props} />
      <Handle type="target" position={Position.Top} className="w-3 h-3 !bg-[#4285F4]" />
      ...
      <Handle type="source" position={Position.Bottom} className="w-3 h-3 !bg-[#4285F4]" />

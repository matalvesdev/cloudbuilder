...
export function CostAnomaliesView() {
  const { anomalies, anomaliesLoading, anomaliesError, fetchAnomalies } =
    useCostStore();
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400">
              Variações inesperadas nos gastos dos serviços
            </p>
          </div>
        </div>
    ...
    <div className="space-y-6">
      ...
      {anomalies.length === 0 ? (
        ...
        <div className="rounded-2xl bg-slate-50 border border-slate-100 p-12 text-center">
          ...
          <p className="text-sm font-medium text-slate-500">
            Nenhuma anomalia com este filtro
          </p>
          <p className="text-xs text-slate-400 mt-1">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/50">
                    <th className="text-left py-3.5 px-4 text-xs font-bold tracking-widest text-slate-400 uppercase">
                      Serviço
                    </th>
                    <th className="text-left py-3.5 px-4 text-xs font-bold tracking-widest text-slate-400 uppercase">
                      Data
                ...
                <tbody>
                  {filteredAnomalies.map((anomaly, idx) => (
                    <AnomalyRow
                      key={`${anomaly.serviceName}-${anomaly.date}-${idx}`}
                      anomaly={anomaly}
                    />
                  ))}
                </tbody>

...
function ActivityHealthSummary() {
  ...
  const stats = useMemo(() => {
    ...
    const deploysOk = deployEvents.filter(
      (e: ActivityEvent) => e.type === "deploy_success",
    ).length;
    ...
    const driftEvents = events.filter(
      (e: ActivityEvent) => e.type === "drift_detected",
    ).length;
    const violations = events.filter(
      (e: ActivityEvent) => e.type === "compliance_violation",
    ).length;
    const optimizations = events.filter(
      (e: ActivityEvent) =>
        e.type === "cost_optimization" || e.type === "cost_saving",
    ).length;
...
function OptimizationSavingsChart() {
  ...
  const data = optimizations
    ...
    .map((o) => ({
      name:
        o.resourceName.length > 14
          ? o.resourceName.slice(0, 12) + "…"
          : o.resourceName,
      savings: o.savings,
...
function DeployTimelineChart() {
  ...
  const data = useMemo(() => {
    ...
    for (const dep of deployments) {
      const depDate = new Date(dep.startedAt).toLocaleDateString("pt-BR", {
        day: "2-digit",
        month: "2-digit",
      });

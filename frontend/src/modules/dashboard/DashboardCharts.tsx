...
/* ─── Chart 1: Monthly Cost Trend (LineChart) ───────────────────────── */
function CostTrendChart() {
  ...
  return (
    <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-5 space-y-3">
      <div className="flex items-center gap-2">
        <span className="w-2 h-2 rounded-full bg-brand-lime" />
        <h3 className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Tendência de Custos</h3>
      </div>
      <ResponsiveContainer width="100%" height={220}>
        <LineChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
          <XAxis dataKey="name" tick={{ fontSize: 10 }} stroke="#94a3b8" />
          <YAxis tick={{ fontSize: 10 }} stroke="#94a3b8" tickFormatter={(v) => `$${(v / 1000).toFixed(1)}k`} />
          <Tooltip
...
/* ─── Chart 2: Cost by Provider (PieChart com Donut) ────────────────── */
function CostByProviderChart() {
  ...
  return (
    <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-5 space-y-3">
      <div className="flex items-center gap-2">
        <span className="w-2 h-2 rounded-full bg-brand-lime" />
        <h3 className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Distribuição por Provedor</h3>
      </div>
...
/* ─── Chart 3: Cost by Service (BarChart) ────────────────────────────── */
function CostByServiceChart() {
  ...
  return (
    <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-5 space-y-3">
      <div className="flex items-center gap-2">
        <span className="w-2 h-2 rounded-full bg-brand-lime" />
        <h3 className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Custos por Serviço</h3>
      </div>
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={data} layout="vertical" margin={{ left: 20 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" horizontal={false} />
          <XAxis type="number" tick={{ fontSize: 10 }} stroke="#94a3b8" tickFormatter={(v) => `$${v}`} />
          <YAxis type="category" dataKey="name" tick={{ fontSize: 11 }} stroke="#94a3b8" width={65} />
          <Tooltip
            contentStyle={{ borderRadius: 12, border: '1px solid #e2e8f0', fontSize: 12 }}
...
/* ─── Chart 4: Provider Resource Distribution (BarChart) ─────────────── */
  ...
  if (total === 0) {
    return (
      <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-5 space-y-3">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-brand-lime" />
          <h3 className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Recursos por Provedor</h3>
        </div>
...
/* ─── Chart 5: Activity Health Summary ───────────────────────────────── */
function ActivityHealthSummary() {
  ...
  return (
    <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-5 space-y-3">
      <div className="flex items-center gap-2">
        <span className="w-2 h-2 rounded-full bg-brand-lime" />
        <h3 className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Resumo de Atividade</h3>
      </div>
            ...
            <div key={item.label} className={cn('rounded-xl p-3 text-center', item.bg)}>
              ...
              {isOkFail && item.value > 0 && (
                <div className="flex items-center justify-center gap-0.5 mt-1">
                  <span className="text-[8px] text-green-600 font-bold">{(item as any).ok}✓</span>
                  {(item as any).fail > 0 && <span className="text-[8px] text-red-600 font-bold">{(item as any).fail}✗</span>}
                </div>
              )}
            </div>
          )
...
/* ─── Chart 6: Recent Cost Optimization Bar ──────────────────────────── */
  ...
  if (data.length === 0) {
    return (
      <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-5 space-y-3">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-brand-lime" />
          <h3 className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Oportunidades de Economia</h3>
        </div>
        <div className="flex items-center justify-center h-[180px] text-slate-400 text-sm">
    ...
    <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-5 space-y-3">
      ...
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={data} layout="vertical" margin={{ left: 10 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" horizontal={false} />
          <XAxis type="number" tick={{ fontSize: 10 }} stroke="#94a3b8" tickFormatter={(v) => `$${v}`} />
          <YAxis type="category" dataKey="name" tick={{ fontSize: 10 }} stroke="#94a3b8" width={65} />
          <Tooltip
            contentStyle={{ borderRadius: 12, border: '1px solid #e2e8f0', fontSize: 12 }}
...
/* ─── Chart 7: Deploy Status Timeline (AreaChart) ────────────────────── */
function DeployTimelineChart() {
  ...
  return (
    <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-5 space-y-3">
      <div className="flex items-center gap-2">
        <span className="w-2 h-2 rounded-full bg-brand-lime" />
        <h3 className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Deploys (7 dias)</h3>
      </div>

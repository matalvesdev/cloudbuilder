...
function StatCard({
  ...
  return (
    <div className="group bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all duration-200 p-5">
      ...
      <p className="text-2xl font-bold text-brand-navy font-display tracking-tight">
        {value}
      </p>
      {sub && (
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {description && (
          <p className={cn("text-xs mt-0.5 line-clamp-2", c.desc)}>
            {description}
          </p>
        )}
        {link && onNavigate && (
          <button
            onClick={() => onNavigate(NAV_MODULES[link.module] || "dashboard")}
            className={cn(
              "mt-1.5 text-xs font-bold underline underline-offset-2 hover:no-underline",
              c.link,
            )}
          >
            {link.label} →
...
export function DashboardModule() {
  ...
  const [quickModal, setQuickModal] = useState<
    "canvas" | "deploy" | "ai" | "finops" | "observability" | null
  >(null);

  const needsSetup = credentials.length === 0;
  const hasEnvSetup = environments.length > 0;
  const wizardDone =
    localStorage.getItem("cloudbuilder-wizard-done") === "true";
  ...
  const driftEvents = useMemo(
    () => activityEvents.filter((e) => e.type === "drift_detected"),
    [activityEvents],
  );
  ...
  const complianceEvents = useMemo(
    () =>
      activityEvents.filter((e) =>
        ["compliance_violation", "compliance_fixed", "compliance_ok"].includes(
          e.type,
        ),
      ),
    [activityEvents],
  ...
  const approvalEvents = useMemo(
    () => activityEvents.filter((e) => e.type === "approval_requested"),
    [activityEvents],
  );
  ...
  const pendingTodos = useMemo<TodoItem[]>(() => {
    ...
    if (needsSetup) {
      items.push({
        ...
        description:
          "Nenhuma credencial configurada — você precisa de uma para provisionar infraestrutura.",
        icon: Key,
      ...
      items.push({
        ...
        description:
          "Credenciais OK, mas você precisa de ao menos um ambiente para fazer deploy.",
        icon: Server,
        action: () => {
    ...
    if (totalResources === 0 && !needsSetup) {
      items.push({
        ...
        description:
          "Seu canvas está vazio. Adicione componentes de infraestrutura para começar.",
        icon: Palette,
    ...
    if (deployEvents.some((e) => e.type === "deploy_fail")) {
      items.push({
        ...
        description:
          "Um ou mais deploys falharam. Verifique os logs para diagnóstico.",
        icon: AlertTriangle,
        action: () => setActiveModule("provisioning"),
        actionLabel: "Ver",
        priority: "high",
  ...
  function UnifiedBody() {
    const alertCount =
      driftEvents.length +
      complianceEvents.filter((e) => e.severity === "error").length;
    const successRate =
      deployEvents.length > 0
        ? Math.round(
            (deployEvents.filter((e) => e.type === "deploy_success").length /
              deployEvents.length) *
              100,
          )
        : null;
          ...
          <div className="lg:col-span-7">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <StatCard
                label="Provedores"
                value={String(
                  Object.values(providerCounts).filter(Boolean).length || 1,
                )}
                sub={
                  totalResources > 0
                    ? `${totalResources} recurso(s)`
                    : undefined
                }
                icon={Cloud}
              />
              <StatCard
                label="Ambientes"
                value={String(environments.length || 1)}
                sub={
                  deployedEnvs.length > 0
                    ? `${deployedEnvs.length} ativo(s)`
                    : undefined
                }
                icon={Server}
              />
              <StatCard
                ...
                sub={
                  activeDeployments.length > 0
                    ? `${activeDeployments.length} em andamento`
                    : undefined
                }
                icon={Rocket}
              />
              <StatCard
                ...
                sub={totalResources > 0 ? "por mês" : "Sem recursos"}
                icon={DollarSign}
              />
              <StatCard
                label="Alertas"
              ...
              <StatCard
                ...
                sub={`${totalConnections} conexões`}
                icon={Cpu}
              />
              <StatCard
                label="Disponibilidade"
                value={uptimeLabel}
                sub={health ? "última verificação" : "sem dados"}
                icon={Shield}
              />
              <StatCard
                label="Sucesso"
                value={successRate !== null ? `${successRate}%` : "—"}
                sub={successRate !== null ? "taxa de deploy" : "sem deploys"}
                icon={CheckCircle2}
              />
              <StatCard
                label="Serviços"
                value={String(deployedEnvs.length || 1)}
                sub={
          ...
          <div className="lg:col-span-5">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
              <SectionHeader
                ...
                badge={
                  activityEvents.length > 0 ? activityEvents.length : undefined
                }
              />
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          {event.link && (
                            <button
                              onClick={() =>
                                setActiveModule(
                                  NAV_MODULES[event.link!.module] || "canvas",
                                )
                              }
          ...
          <div className="lg:col-span-5">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <SectionHeader
                ...
                badge={
                  pendingTodos.length > 0 ? pendingTodos.length : undefined
                }
              />
              ...
              {totalResources === 0 ? (
                ...
                <>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
                    {(["aws", "azure", "gcp", "k8s"] as const).map((p) => {
                      ...
                      return count > 0 ? (
                        ...
                        <div
                          key={p}
                          className="rounded-xl px-3 py-2.5 text-center bg-slate-50 border border-slate-100"
                        >
                          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-300">
                            {p}
                          </p>
                          <p className="text-xl font-bold font-display text-slate-300 mt-0.5">
                            0
                          </p>
                        </div>
                      );
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    ...
                    {(() => {
                      const s = activityEvents.find(
                        (e) => e.type === "design_save",
                      );
                      return s ? (
                ...
                <div className="space-y-2 max-h-[280px] overflow-y-auto pr-1 -mr-1">
                  {driftEvents.slice(0, 1).map((evt) => (
                    <AlertCard
                      ...
                      link={
                        evt.link
                          ? { label: evt.link.label, module: evt.link.module }
                          : undefined
                      }
                      onNavigate={setActiveModule}
                    />
                  ...
                  {complianceEvents.slice(0, 1).map((evt) => (
                    <AlertCard
                      key={evt.id}
                      icon={
                        evt.severity === "error" ? ShieldAlert : ShieldCheckIcon
                      }
  ...
  const healthStatus =
    health === "UP" ? "healthy" : health === "DEGRADED" ? "degraded" : "down";
  const statusColor =
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-slate-50 to-white">
          ...
          {needsSetup && !showSetupWizard && wizardDone && (
            <div className="bg-gradient-to-r from-brand-navy to-[#1a2a5e] rounded-2xl p-5 text-white shadow-md">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/70 mt-0.5">
                      Configure suas credenciais de nuvem para começar a
                ...
                >
                  <Key className="w-4 h-4" /> Configurar{" "}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-800">
                    Ambientes não configurados
                  </p>
                  <p className="text-xs text-amber-600 mt-0.5">
                    Crie um ambiente para poder fazer deploy da sua
                    infraestrutura.
                  </p>
                </div>
              </div>
      ...
      {/* Novo Design Modal */}
      <Dialog
        open={quickModal === "canvas"}
        onOpenChange={() => setQuickModal(null)}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Novo Design</DialogTitle>
            <DialogDescription>
              Criar um novo design de infraestrutura
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-2">
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Comece do zero arrastando recursos
                </p>
              </div>
            </button>
            <button
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Arquiteturas pré-prontas do catálogo
                </p>
              </div>
            </button>
            <button
              onClick={() => {
                localStorage.setItem("cloudbuilder-import-mode", "true");
                setQuickModal(null);
                setActiveModule("provisioning");
              }}
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Terraform, CloudFormation ou Kubernetes
                </p>
              </div>
            </button>
          </div>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Ambientes e deploys disponíveis
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Diagnóstico inteligente e runbooks automatizados
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 py-2">
            <button
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Diagnosticar incidentes e obter recomendações
                </p>
              </div>
            </button>
            <button
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Histórico e resolução de incidentes
                </p>
              </div>
            </button>
          </div>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Visão rápida dos custos de infraestrutura
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-brand-navy to-[#1a2a5e] text-white">
              <div>
                ...
                <p className="text-xs text-white/50 mt-0.5">
                  Mês atual • {totalResources} recursos
                </p>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-purple-50 border border-purple-200/60">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-purple-800">
                    {costEvents.length} oportunidade(s) de economia
                  </p>
                </div>
              </div>
            )}
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Métricas, alertas e monitoramento da infraestrutura
            </DialogDescription>
          </DialogHeader>

    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        {d.alertCount > 0 && (
          <Badge
            variant="destructive"
            className="ml-auto text-[10px] h-5 px-1.5 gap-1"
          >
...
function edgeStyle(status: string) {
  ...
  if (status === "degraded")
    return { stroke: "#eab308", strokeWidth: 2, animated: true };
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3 rounded-xl bg-slate-50 text-center">
            ...
            <p
              className={cn(
                "text-sm font-bold",
                detail.alertCount > 0 ? "text-red-600" : "text-green-600",
              )}
            >
              {detail.alertCount}
            </p>
...
export function ServiceMapView() {
  ...
  const [selectedNode, setSelectedNode] = useState<NodeDetailResponse | null>(
    null,
  );
    ...
    async (id: string) => {
      ...
      try {
        ...
        const flowNodes: Node[] = res.nodes.map((n, i) => ({
          ...
          type: "serviceMapNode",
          position: {
            x: n.positionX || (i % 4) * 250,
            y: n.positionY || Math.floor(i / 4) * 180,
          },
          data: {
            label: n.componentDefinitionId
              .replace(/_/g, " ")
        ...
        const flowEdges: Edge[] = res.edges.map((e) => ({
          ...
          markerEnd: {
            ...
            color: edgeStyle(res.overallStatus).stroke,
          },
  ...
  const handleNodeClick = useCallback(
    async (nodeId: string) => {
      ...
      setLoadingNode(true);
      try {
        const detail = await api.get<NodeDetailResponse>(
          `/service-map/${canvasId}/nodes/${nodeId}`,
        );
  ...
  return (
    <div className="relative space-y-4">
      ...
      {data && nodes.length > 0 ? (
        <div
          className="bg-white rounded-3xl card-shadow border border-slate-100 overflow-hidden relative"
          style={{ height: 520 }}
        >
          <ReactFlow
            nodes={nodes}
        ...
        <div className="flex flex-col items-center justify-center h-[300px] bg-white rounded-3xl card-shadow border border-slate-100">
          ...
          <p className="text-sm text-slate-400">
            Canvas vazio — adicione componentes ao design para ver o service map
          </p>
        </div>
      ) : null}
    </div>

...
const SEVERITY_CONFIG: Record<string, { bg: string; icon: typeof AlertCircle }> = {
...
export function IncidentsView() {
  ...
  const { data: streamData } = useSSE<IncidentDTO[]>('/observability/incidents/stream', 'incidents')
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-lg font-bold text-brand-navy font-display">Incidentes</h2>
      </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {activeIncidents.map((inc) => {
              const config = SEVERITY_CONFIG[inc.severity] || SEVERITY_CONFIG.info
              const Icon = config.icon
              return (
                <div key={inc.id} className={cn('flex items-start gap-3 rounded-xl border p-4', config.bg)}>
                  <Icon className="h-5 w-5 text-red-500 mt-0.5 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-medium text-brand-navy">{inc.title}</p>
                  ...
                  <div className="flex gap-1">
                    <Button size="sm" variant="outline" onClick={() => handleAcknowledge(inc.id)}>
                      <Eye className="h-3.5 w-3.5 mr-1" />
                      Reconhecer
                    </Button>
                    <Button size="sm" variant="outline" className="text-green-600" onClick={() => handleResolve(inc.id)}>
        ...
        {resolvedIncidents.length === 0 ? (
          ...
          <div className="space-y-2">
            {resolvedIncidents.map((inc) => (
              <div key={inc.id} className="flex items-start gap-3 rounded-xl border border-slate-100 p-4 bg-white">
                <CheckCircle2 className="h-5 w-5 text-green-500 mt-0.5 shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-slate-600">{inc.title}</p>
                  <p className="text-xs text-slate-400">
                    Resolvido em {inc.resolvedAt ? new Date(inc.resolvedAt).toLocaleString('pt-BR') : '—'}
                  </p>
                </div>
              </div>
            ))}

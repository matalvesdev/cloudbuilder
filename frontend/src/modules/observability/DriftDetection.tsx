...
const severityColors: Record<
  DriftSeverity,
  { bg: string; text: string; icon: typeof ShieldAlert }
> = {
...
function DriftSummaryCards({ report }: { report: DriftReport }) {
  ...
  const critical = report.resources.filter(
    (r) => r.severity === "CRITICAL" && r.status === "DETECTED",
  ).length;
  const modified = report.resources.filter(
    (r) => r.driftType === "MODIFIED" && r.status === "DETECTED",
  ).length;
  const added = report.resources.filter(
    (r) => r.driftType === "ADDED" && r.status === "DETECTED",
  ).length;
  const removed = report.resources.filter(
    (r) => r.driftType === "REMOVED" && r.status === "DETECTED",
  ).length;
    ...
    <div className="grid grid-cols-4 gap-4">
      <div className="bg-white rounded-3xl p-5 card-shadow border border-slate-100 flex items-center gap-4">
        <div
          className={cn(
            "rounded-xl p-3",
            critical > 0 ? "bg-red-50" : "bg-ice-blue",
          )}
        >
          <ShieldAlert
            className={cn(
              "h-5 w-5",
              critical > 0 ? "text-red-500" : "text-brand-navy",
            )}
          />
        </div>
        <div>
          ...
          <p
            className={cn(
              "text-2xl font-bold",
              critical > 0 ? "text-red-600" : "text-brand-navy",
            )}
          >
            {total}
          </p>
...
function PropertyDiffRow({
  ...
  return (
    <div className="flex items-center gap-2 text-xs font-mono">
      ...
      <span className="text-slate-400 line-through flex-1 truncate">
    ...
    <div
      className={cn(
        ...
        resolved
          ...
          : "bg-white card-shadow border-slate-100",
      )}
    >
      <button
        type="button"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span
              className={cn(
                ...
                resolved ? "text-slate-400" : "text-brand-navy",
              )}
            >
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <span
              className={cn(
                "text-xs font-medium",
                providerColor(resource.provider),
              )}
            >
              {resource.provider}
            </span>
            ...
            <SevIcon
              className={cn(
                "h-3 w-3 ml-1",
                severityColors[resource.severity].text,
              )}
            />
          </div>
        </div>
          ...
          <div className="border-t border-slate-100 pt-3 space-y-1.5">
            {resource.propertyChanges.length === 0 ? (
              <p className="text-xs text-slate-400 italic">
                Nenhuma propriedade com diferença
              </p>
            ) : (
      ...
      <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-6">
        <div className="flex flex-col items-center justify-center py-12 text-slate-400">
          ...
          <p className="text-xs mt-1">
            Expanda um recurso acima para ver o comparativo lado a lado
          </p>
        </div>
      </div>
    );
            ...
            return (
              <div key={event.id} className="flex items-start gap-3">
                ...
                >
                  <div
                    className={cn(
                      "w-1.5 h-1.5 rounded-full m-auto mt-[3px]",
                      resolved ? "bg-slate-300" : colors.dot,
                    )}
                  />
                </div>
...
export function DriftDetection() {
  ...
  const activeReports = useMemo(
    () =>
      reports.filter((r) =>
        r.resources.some((res) => res.status === "DETECTED"),
      ),
    [reports],
  );
  const currentReport =
    reports[activeReportIdx] || activeReports[0] || reports[reports.length - 1];
  const activeResources =
    currentReport?.resources.filter((r) => r.status === "DETECTED") || [];
  const resolvedCount =
    currentReport?.resources.filter((r) => r.status !== "DETECTED").length || 0;
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-12 flex flex-col items-center justify-center text-slate-400">
          ...
          <p className="text-sm mt-1">
            Toda a infraestrutura está sincronizada com o design do canvas.
          </p>
          <Button
            variant="outline"
            size="sm"
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Última detecção:{" "}
            {new Date(currentReport.detectedAt).toLocaleString("pt-BR")} — v
            {currentReport.canvasVersion} — {currentReport.environmentId}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button
          ...
          >
            <RefreshCw
              className={cn("h-3.5 w-3.5", isDetecting && "animate-spin")}
            />
            Detectar
          </Button>
          <Button
      ...
      {activeResources.length > 0 && (
        <div className="flex items-center gap-2 px-1">
          ...
          <Button
            ...
            onClick={() =>
              currentReport && remediateAllDrifts(currentReport.id)
            }
          >
            <RefreshCw className="h-3 w-3" />
            Remediar Todos
          ...
          <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-6 space-y-3">
            ...
            <div className="space-y-2">
              {reports.slice(0, 10).map((r, i) => {
                const active = r.resources.filter(
                  (res) => res.status === "DETECTED",
                ).length;
                return (
                  <button
                    key={r.id}
                  ...
                  >
                    <div
                      className={cn(
                        "w-2 h-2 rounded-full shrink-0",
                        active > 0 ? "bg-red-400" : "bg-emerald-400",
                      )}
                    />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-brand-navy truncate">
                        Relatório #{reports.length - i}
                      </p>

  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-lg font-bold text-brand-navy font-display">SLO — Service Level Objectives</h2>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-slate-400">
          ...
          <p className="text-xs mt-1">Configure definições de SLO no backend para começar</p>
        </div>
      ) : (
          ...
          {slos.map((slo) => (
            <div
              ...
              className={cn(
                ...
                slo.status === 'BREACHED' ? 'border-red-200' : 'border-slate-100'
              )}
            >
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-brand-navy font-display">{slo.name}</h3>
                <span className={cn(
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs">
                  ...
                  <span className={cn('font-mono font-medium', getBudgetColor(slo.currentSliPct))}>
                    {slo.currentSliPct.toFixed(2)}%
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-slate-400">Target</span>
                  <span className="font-mono text-slate-600">{slo.targetPct}%</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-slate-400">Error Budget</span>
                  <span className={cn('font-mono font-medium', getBudgetColor(slo.errorBudgetPct))}>
              ...
              <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className={cn('h-full rounded-full transition-all', getProgressColor(slo.errorBudgetPct))}

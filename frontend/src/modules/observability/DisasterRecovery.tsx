...
const RESOURCE_REPLICATION_LABELS: Record<
  ReplicationResourceType,
  { label: string; icon: typeof Server; desc: string }
> = {
  rds: {
    label: "RDS Read Replica",
    icon: Database,
    desc: "Réplica de leitura entre regiões",
  },
  s3: {
    label: "S3 Cross-Region",
    icon: Server,
    desc: "Replicação entre regiões",
  },
  route53: {
    label: "Route53 Failover",
    icon: Globe,
    desc: "Roteamento de failover DNS",
  },
};
...
const FAILOVER_STEPS: FailoverSimulationStep[] = [
  {
    step: 1,
    name: "Verificando saúde da região primária",
    status: "pending",
    duration_seconds: 3,
  },
  {
    step: 2,
    name: "Promovendo réplica de leitura RDS",
    status: "pending",
    duration_seconds: 8,
  },
  {
    step: 3,
    name: "Ativando bucket S3 de DR",
    status: "pending",
    duration_seconds: 5,
  },
  {
    step: 4,
    name: "Redirecionando DNS Route53",
    status: "pending",
    duration_seconds: 12,
  },
  {
    step: 5,
    name: "Verificando health check da aplicação",
    status: "pending",
    duration_seconds: 10,
  },
  {
    step: 6,
    name: "Validando integridade dos dados",
    status: "pending",
    duration_seconds: 9,
  },
];
      ...
      {config ? (
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-400">
            <span className="inline-flex items-center gap-1">
              ...
              Último teste:{" "}
              {config.lastTestDate
                ? new Date(config.lastTestDate).toLocaleDateString("pt-BR")
                : "Nunca"}
            </span>
            <span
              className={cn(
            ...
            <span className="inline-flex items-center gap-1">
              ...
              Teste automático:{" "}
              {config.autoTestSchedule === "weekly"
                ? "Semanal"
                : config.autoTestSchedule === "monthly"
                  ? "Mensal"
                  : "Desligado"}
            </span>
          </div>
        ...
        <div className="flex flex-col items-center justify-center py-8 text-center">
          ...
          <p className="text-xs text-slate-400">
            Nenhuma configuração DR ativa. Configure o DR para começar.
          </p>
        </div>
      )}
    </div>
            ...
            <div className="flex-1">
              ...
              {step.status === "completed" && (
                <p className="text-[10px] text-green-600">
                  Concluído em {step.duration_seconds}s
                </p>
              )}
            </div>
            {step.status === "pending" && (
      ...
      {allCompleted && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
          ...
          <p className="text-[11px] text-green-600 mt-1">
            RTO real: {totalDuration}s | RPO real: ~32s — dentro dos objetivos
            configurados
          </p>
        </div>
      )}
      ...
      {results.map((test) => (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold text-brand-navy">
                {new Date(test.testedAt).toLocaleDateString("pt-BR", {
                  day: "numeric",
                  month: "long",
                  year: "numeric",
                })}
              ...
              <p className="text-[10px] text-slate-400">
                RTO: {test.rto_actual}s · RPO: {test.rpo_actual}s · Duração:{" "}
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {(
              Object.entries(RESOURCE_REPLICATION_LABELS) as [
                ReplicationResourceType,
                (typeof RESOURCE_REPLICATION_LABELS)["rds"],
              ][]
            ).map(([key, cfg]) => {
              const Icon = cfg.icon;
              return (
...
export function DisasterRecovery() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Failover entre regiões e continuidade de negócio
            </p>
          </div>
        </div>
        <button
      ...
      {showSetup && <DrSetupForm onSave={handleSaveConfig} />}
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                {testResults.length} testes realizados
              </p>
            </div>
          </div>

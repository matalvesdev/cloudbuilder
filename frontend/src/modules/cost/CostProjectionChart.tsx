...
function ProjectionSVG({ data }: ChartProps) {
  ...
  const { minVal, maxVal, xScale, yScale, linePath, areaPath } = useMemo(() => {
    if (data.length === 0) {
      return { minVal: 0, maxVal: 0, xScale: () => 0, yScale: () => 0, linePath: '', areaPath: '' }
    }

    const allValues = data.flatMap((d) => [d.projectedAmount, d.lowerBound, d.upperBound])
    ...
    const yS = (v: number) => CHART_MARGIN.top + innerH - ((v - min) / (max - min)) * innerH

    // Line path for projected amount
    let line = data.map((d, i) => `${i === 0 ? 'M' : 'L'}${xS(i)},${yS(d.projectedAmount)}`).join(' ')
    // Area path for confidence interval (lower + upper)
    const upperPoints = data.map((d, i) => `${xS(i)},${yS(d.upperBound)}`).join(' L')
    const lowerPoints = data.map((d, i) => `${xS(i)},${yS(d.lowerBound)}`).join(' L')
  ...
  const xTicks = useMemo(() => {
    ...
    for (let i = 0; i < data.length; i += step) {
      ticks.push({ label: formatShortDate(data[i].date), x: xScale(i), idx: i })
    }
    // Always include last
    const last = data.length - 1
    if (ticks.length === 0 || ticks[ticks.length - 1].idx !== last) {
      ticks.push({ label: formatShortDate(data[last].date), x: xScale(last), idx: last })
    }
    return ticks
  }, [data, xScale])

  const latestProjection = data.length > 0 ? data[data.length - 1].projectedAmount : 0
  const earliestProjection = data.length > 0 ? data[0].projectedAmount : 0
  const trendPct = earliestProjection > 0
    ? ((latestProjection - earliestProjection) / earliestProjection * 100).toFixed(1)
    : '0.0'
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-4 text-center space-y-1">
          ...
          <p className="text-xl font-bold text-brand-navy">{formatCurrency(latestProjection)}</p>
        </div>
        <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-4 text-center space-y-1">
          <p className="text-xs font-bold tracking-widest text-slate-400 uppercase">Início</p>
          <p className="text-xl font-bold text-brand-navy">{formatCurrency(earliestProjection)}</p>
        </div>
        <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-4 text-center space-y-1">
        ...
        <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-4 text-center space-y-1">
          <p className="text-xs font-bold tracking-widest text-slate-400 uppercase">Período</p>
      ...
      <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-6">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="text-xs font-bold tracking-widest text-slate-400 uppercase">Projeção de Custos</h2>
        </div>
        <svg
        ...
        >
          ...
          {data.length > 0 && (
            <path
              d={data.map((d, i) => `${i === 0 ? 'M' : 'L'}${xScale(i)},${yScale(d.upperBound)}`).join(' ')}
              fill="none"
              stroke="#0a1128"
              strokeWidth={1}
              strokeDasharray="6,3"
          ...
          {data.length > 0 && (
            <path
              d={data.map((d, i) => `${i === 0 ? 'M' : 'L'}${xScale(i)},${yScale(d.lowerBound)}`).join(' ')}
              fill="none"
              stroke="#0a1128"
              strokeWidth={1}
              strokeDasharray="6,3"
        ...
        <div className="flex items-center justify-center gap-6 mt-4 pt-4 border-t border-slate-100">
          ...
          <div className="flex items-center gap-2">
            <div className="w-6 h-0.5 bg-brand-navy/30 rounded-full" style={{ borderTop: '2px dashed rgba(10,17,40,0.3)' }} />
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-brand-navy font-display">Projeção de Custos</h2>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-2xl bg-red-50 border border-red-200 p-6 text-center">
          ...
          <p className="text-sm font-medium text-red-700 mb-1">Erro ao carregar projeção</p>
    ...
    <div className="space-y-6">
      ...
      {projection.length === 0 ? (
        <div className="rounded-2xl bg-slate-50 border border-slate-100 p-12 text-center">
          ...
          <p className="text-sm font-medium text-slate-500">Nenhum dado de projeção disponível</p>

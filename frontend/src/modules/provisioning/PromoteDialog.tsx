...
import {
  ArrowRight, CheckCircle2, XCircle, Loader2, Shield, Clock, User,
  FileCode2, GitCompare, AlertTriangle, ArrowUpFromLine,
} from 'lucide-react'
...
function canPromoteTo(source: Environment, target: Environment): boolean {
  return (ENV_TYPE_ORDER[source.type] ?? 0) < (ENV_TYPE_ORDER[target.type] ?? 0)
}

function getAvailableTargets(source: Environment, all: Environment[]): Environment[] {
...
export function PromoteDialog({ onClose }: PromoteDialogProps) {
  ...
  const { promotions, approvals: storeApprovals, addPromotion, getPendingApprovals } = usePromotionStore()
  const { teamMembers, approvalRules, requestApproval, getEnvsRequiringApproval } = useApprovalStore()
  ...
  const canPromote = sourceEnv && targetEnv && latestSourceDeploy?.status === 'success'
  ...
  const handlePromote = async () => {
    ...
    const approvalConfigured = needsApproval || (envRule?.requiresApproval ?? false)
    ...
    try {
      ...
      if (approvalConfigured) {
        ...
        usePromotionStore.getState().updatePromotionStatus(
          promId,
          'deployed',
      ...
      >
        ...
        <div className="p-6 border-b border-slate-100">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-brand-navy font-display">Promover Design</h2>
              ...
              {(['select', 'review', 'confirm'] as Step[]).map((s, i) => (
                <div key={s} className="flex items-center gap-1">
                  <span className={cn(
                    ...
                    ['select', 'review', 'confirm'].indexOf(step) > i ? 'bg-green-500 text-white' :
                  ...
                  {i < 2 && <div className={cn(
                    'w-6 h-px',
                    ['select', 'review', 'confirm'].indexOf(step) > i ? 'bg-green-500' : 'bg-slate-200'
                  )} />}
                </div>
              ))}
            </div>
        ...
        {step === 'select' && (
          <div className="p-6 space-y-5">
            <p className="text-sm font-semibold text-brand-navy">Selecione a origem e o destino da promoção</p>
            ...
            <div>
              ...
              >
                ...
                {environments.map((env) => {
                  const envDeploys = deployments.filter((d) => d.environmentId === env.id)
                  const hasDeploy = envDeploys.some((d) => d.status === 'success')
                  return (
                    <option key={env.id} value={env.id} disabled={!hasDeploy}>
                      {env.name} ({ENVIRONMENT_TYPE_LABELS[env.type]}) {!hasDeploy ? '— sem deploy' : `— v${env.canvasVersion}`}
                    </option>
                  )
                })}
              </select>
              {sourceEnv && latestSourceDeploy && (
                <div className="mt-2 flex items-center gap-2 text-xs text-slate-500">
                  ...
                  Último deploy: <span className="font-semibold text-brand-navy">{latestSourceDeploy.version}</span>
              ...
              >
                ...
                {availableTargets.map((env) => (
                  <option key={env.id} value={env.id}>
                    {env.name} ({ENVIRONMENT_TYPE_LABELS[env.type]} — v{env.canvasVersion})
                  </option>
                ))}
              </select>
              {targetEnv && (
                <div className="mt-2 flex items-center gap-2 text-xs text-slate-500">
                  ...
                  Versão atual: <span className="font-semibold text-brand-navy">v{targetEnv.canvasVersion}</span>
                  {latestTargetDeploy && (
                    <>
                      <span className="text-slate-300">|</span>
            ...
            {(targetEnv?.type === 'production' || approvalRules.find((r) => r.environmentId === targetEnv?.id)?.requiresApproval) && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                <div className="flex items-start gap-3">
                  <Shield className="w-5 h-5 text-amber-600 mt-0.5 shrink-0" />
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-amber-800">
                      {targetEnv?.type === 'production' ? 'Ambiente de Produção' : 'Portão de Aprovação'}
                    </p>
        ...
        {step === 'review' && sourceEnv && targetEnv && (
          <div className="p-6 space-y-5">
            <p className="text-sm font-semibold text-brand-navy">Revise as diferenças entre os ambientes</p>

            {/* Environment Comparison */}
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-slate-50 rounded-xl p-4">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Origem</p>
                <p className="text-sm font-bold text-brand-navy">{sourceEnv.name}</p>
                <p className="text-xs text-slate-500 capitalize">{ENVIRONMENT_TYPE_LABELS[sourceEnv.type]}</p>
              </div>
              ...
              <div className="bg-slate-50 rounded-xl p-4">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Destino</p>
                <p className="text-sm font-bold text-brand-navy">{targetEnv.name}</p>
                <p className="text-xs text-slate-500 capitalize">{ENVIRONMENT_TYPE_LABELS[targetEnv.type]}</p>
              </div>
            </div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-slate-50">
                    <th className="text-left p-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Item</th>
                    <th className="text-center p-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Origem</th>
                    <th className="text-center p-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Destino</th>
                    <th className="text-center p-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Diferença</th>
                  </tr>
                </thead>
                <tbody>
                  {diffItems.map((item) => (
                    <tr key={item.label} className="border-t border-slate-100">
                      <td className="p-3 text-sm font-medium text-brand-navy">{item.label}</td>
                      <td className="p-3 text-center text-sm text-slate-600">{item.source}</td>
                      <td className="p-3 text-center text-sm text-slate-600">{item.target}</td>
                      <td className="p-3 text-center">
                        {'diff' in item && item.diff !== undefined ? (
                          ...
                          <span className="text-xs font-bold text-green-600">Igual</span>
                        ) : (
                          <span className="text-xs font-bold text-amber-600">Diferente</span>
                        )}
                      </td>
                    </tr>
                  ))}
                  <tr className="border-t border-slate-100">
                    <td className="p-3 text-sm font-medium text-brand-navy">Conexões</td>
                    <td className="p-3 text-center text-sm text-slate-600">{edges.length}</td>
                    <td className="p-3 text-center text-sm text-slate-600">{latestTargetDeploy ? '—' : '—'}</td>
                    <td className="p-3 text-center">
                      <span className="text-xs font-bold text-green-600">Novo</span>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div className="bg-ice-blue/50 rounded-xl p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-brand-navy">Snapshot do Design</p>
                  <p className="text-xs text-slate-500 mt-0.5">
                    O design atual <strong>"{canvasName}"</strong> (v{canvasVersion}) será copiado para {targetEnv.name}.
                    {(requiresApproval && targetEnv.type === 'production') || approvalRules.find((r) => r.environmentId === targetEnv?.id)?.requiresApproval ? (
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              >
                {promoting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <ArrowUpFromLine className="w-3.5 h-3.5" />}
                {promoting ? 'Promovendo...' : (requiresApproval && targetEnv.type === 'production') || approvalRules.find((r) => r.environmentId === targetEnv?.id)?.requiresApproval ? 'Solicitar Promoção' : 'Promover Agora'}
              </button>
            </div>
          </div>
        )}
          ...
          <div className="p-6 space-y-5">
            {promoteError ? (
              <div className="flex items-center gap-3 p-4 rounded-xl bg-red-50 border border-red-200">
                ...
                <div>
                  <p className="text-sm font-semibold text-red-700">Erro na Promoção</p>
                  <p className="text-xs text-red-500">{promoteError}</p>
                </div>
              </div>
            ) : promoteDone && ((requiresApproval && targetEnv?.type === 'production') || approvalRules.find((r) => r.environmentId === targetEnv?.id)?.requiresApproval) ? (
              <div className="flex items-center gap-3 p-4 rounded-xl bg-amber-50 border border-amber-200">
                <Clock className="w-5 h-5 text-amber-500 shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-amber-700">Aprovação Pendente</p>
                  <p className="text-xs text-amber-500">
                    A solicitação de promoção para produção foi enviada para aprovação.
                    O deploy será realizado após a aprovação de um supervisor.
                  </p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 p-4 rounded-xl bg-green-50 border border-green-200">
                ...
                <div>
                  <p className="text-sm font-semibold text-green-700">
                    {targetEnv?.type === 'production' ? 'Promovido para Produção!' : 'Promoção Concluída!'}
                  </p>
                  <p className="text-xs text-green-500">
                    Design <strong>"{canvasName}"</strong> promovido de {sourceEnv?.name} para {targetEnv?.name}.
                    {nodes.length} recursos, versão v{targetEnv ? targetEnv.canvasVersion + 1 : ''}.
                  </p>
                </div>
              </div>
            )}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {pipelinePromotions.slice(0, 10).map((prom) => {
                    const src = environments.find((e) => e.id === prom.sourceEnvId)
                    const tgt = environments.find((e) => e.id === prom.targetEnvId)
                    return (
                      <div key={prom.id} className="flex items-center gap-3 p-3 rounded-lg bg-slate-50 border border-slate-100">
                        <div className={cn(
                          'w-7 h-7 rounded-full flex items-center justify-center',
                          prom.status === 'deployed' ? 'bg-green-100 text-green-600' :
                          prom.status === 'approved' ? 'bg-blue-100 text-blue-600' :
                          prom.status === 'rejected' ? 'bg-red-100 text-red-600' :
                          prom.status === 'failed' ? 'bg-red-100 text-red-600' :
                          'bg-amber-100 text-amber-600'
                        )}>
                          {prom.status === 'deployed' ? <CheckCircle2 className="w-3.5 h-3.5" /> :
                           prom.status === 'pending' ? <Clock className="w-3.5 h-3.5" /> :
                           <XCircle className="w-3.5 h-3.5" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 text-xs">
                            <span className="font-semibold text-brand-navy">{src?.name ?? '—'}</span>
                            <ArrowRight className="w-3 h-3 text-slate-400" />
                            <span className="font-semibold text-brand-navy">{tgt?.name ?? '—'}</span>
                          </div>
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            {new Date(prom.requestedAt).toLocaleString('pt-BR')}
                            {prom.completedAt && ` · ${new Date(prom.completedAt).toLocaleString('pt-BR')}`}
                          </p>
                        </div>
                        <span className={cn(
                          'text-[10px] px-2 py-0.5 rounded-full font-bold border',
                          prom.status === 'deployed' ? 'bg-green-50 text-green-700 border-green-200' :
                          prom.status === 'pending' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                          prom.status === 'approved' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                          'bg-red-50 text-red-700 border-red-200'
                        )}>
                          {prom.status === 'deployed' ? 'Implantado' :
                           prom.status === 'pending' ? 'Pendente' :
                           prom.status === 'approved' ? 'Aprovado' :
                           prom.status === 'rejected' ? 'Rejeitado' :
                           'Falha'}
                        </span>
                      </div>
                    )
            ...
            {pendingApprovals.length > 0 && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-center gap-3">
                ...
                <div className="text-xs text-amber-700">
                  <span className="font-bold">{pendingApprovals.length}</span> promoção

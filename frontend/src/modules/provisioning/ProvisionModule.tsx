...
export function ProvisionModule() {
  ...
  const [deployStatus, setDeployStatus] = useState<
    "idle" | "planning" | "applying" | "done" | "error"
  >("idle");
  ...
  const [deployStep, setDeployStep] = useState<
    "idle" | "plan" | "review" | "applying" | "done" | "error"
  >("idle");
  ...
  const { nodes, edges, canvasName, canvasId, canvasVersion } =
    useCanvasStore();
  ...
  const isDeployed =
    !!selectedEnvId &&
    storeDeployments.some(
      (d) => d.environmentId === selectedEnvId && d.status === "success",
    );
  const lastDeployedVersion = useMemo(() => {
    if (!selectedEnvId) return "";
    const envDeploys = storeDeployments.filter(
      (d) => d.environmentId === selectedEnvId && d.status === "success",
    );
    return envDeploys.length > 0
      ? envDeploys[envDeploys.length - 1].version
      : "";
  ...
  const envsRequiringApproval = useMemo(
  ...
  const userCanApproveSome = useMemo(() => {
    ...
    const approvers = useApprovalStore
      .getState()
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const res = await provisionApi.generateCode(canvasId, engine, {
        nodes: nodes.map((n) => ({
          id: n.id,
          label: n.data?.label,
  ...
  const handlePlan = useCallback(
    async (envId?: string) => {
      ...
      try {
        ...
        const destroy = drifted.filter(
          (r) => r.driftType === "unexpected",
        ).length;
        // If no drift detected, all resources are additions
        const planAdd =
          add > 0 || change > 0 || destroy > 0 ? add : nodes.length;
  ...
  const handleResolveDrift = useCallback(
    async (envId: string) => {
  ...
  const outputsTf =
    ...
    (() => {
      ...
      if (lines.length === 1) {
        lines.push(
          `# ${nodes.length} recursos de ${providers.join(", ") || "diversos provedores"}`,
        );
      }
      return lines.join("\n");
    })();

  const realDeployments = selectedEnvId
    ? storeDeployments.filter((d) => d.environmentId === selectedEnvId)
    : [];
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      addDeployment({
        ...
        planSummary: {
          add: generated.resourceCount || nodes.length,
          change: 0,
          destroy: 0,
        },
      });
    ...
    <div className="p-7 overflow-y-auto flex-1">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="font-display text-[28px] font-bold tracking-tight text-brand-navy">
              Provisionar
            </h1>
            {hasCanvasDesign && (
            ...
            {pendingApprovals.length > 0 && (
              ...
              >
                ...
                {pendingApprovals.length} aprovação
          ...
          <ProtectedAction roles={["admin", "editor"]}>
            <div className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-xl px-3 h-9 shadow-sm">
              ...
              <select
                value={engine}
                onChange={(e) =>
                  setEngine(e.target.value as "terraform" | "opentofu")
                }
          ...
          {selectedEnvironment && (
            <div className="flex items-center gap-2">
              <span
                className={cn(
                  ...
                  selectedEnvironment.status === "ACTIVE"
                    ...
                    : selectedEnvironment.status === "PENDING"
              ...
              <span className="text-xs text-slate-400">
                {selectedEnvironment.provider.toUpperCase()} /{" "}
                {selectedEnvironment.region}
              </span>
              {/* Drift detection badge */}
              {selectedEnvId &&
                driftMap[selectedEnvId] &&
                driftMap[selectedEnvId]!.status === "drifted" && (
                  <button
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-card">
          <h3 className="text-[12px] font-bold uppercase tracking-wide text-slate-400 mb-2.5">
            main.tf
          </h3>
          {loading ? (
              ...
              <div className="flex gap-5 mb-3">
                <div>
                  <span className="text-green-600 font-bold text-[22px] leading-none">
                    {code?.resourceCount || nodes.length || 0}
                  </span>
                ...
                <div>
                  <span className="text-yellow-600 font-bold text-[22px] leading-none">
                    0
                  </span>
                  <span className="text-[11px] text-slate-400 ml-1">
                    alterar
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-card">
          ...
          {realDeployments.length === 0 ? (
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  <th className="text-left p-[10px_14px] text-[11px] font-bold text-slate-400 border-b border-slate-200 uppercase tracking-wide">
                    Versão
                  </th>
                  <th className="text-left p-[10px_14px] text-[11px] font-bold text-slate-400 border-b border-slate-200 uppercase tracking-wide">
                    Status
                  ...
                  <th className="text-right p-[10px_14px] text-[11px] font-bold text-slate-400 border-b border-slate-200 uppercase tracking-wide">
                    Ações
                  </th>
                </tr>
              </thead>
              <tbody>
                {realDeployments.map((d) => (
                  <tr
                    key={d.id}
                    className="hover:bg-ice-blue transition-colors"
                  >

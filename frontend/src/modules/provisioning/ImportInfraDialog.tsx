...
function inferConnections(
  ...
  for (const res of resources) {
    ...
    if (props.subnetId || props.subnetwork) {
      ...
      const target = resources.find(
        (r) => r.id === subnetId || r.name === subnetId,
      );
      if (target) {
        conns.push({ source: target.id, target: res.id, type: "network" });
      }
...
export function ImportInfraDialog({
  open,
  onOpenChange,
}: ImportInfraDialogProps) {
  ...
  const [stateParseResult, setStateParseResult] = useState<
  ...
  const handleStateFileSelect = useCallback(
  ...
  const handleTfDirSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      for (const file of Array.from(files)) {
        ...
        if (content.includes('resource "')) {
          const provider = content.includes("aws_")
            ? ("aws" as ProviderType)
            : content.includes("azurerm_")
              ? ("azure" as ProviderType)
  ...
  const toggleGroup = useCallback(
    (groupResources: ScannedResource[], checked: boolean) => {
      setSelectedResourceIds((prev) => {
        const next = new Set(prev);
    ...
    if (activeTab === "terraform-dir") {
      ...
      for (const file of tfFiles) {
        for (const res of file.resources) {
          if (selectedResourceIds.has(res.id)) {
            const groupName =
              res.resourceType.includes("vpc") ||
              res.resourceType.includes("subnet") ||
              res.resourceType.includes("sg") ||
              res.resourceType.includes("lb") ||
              res.resourceType.includes("igw") ||
              res.resourceType.includes("route")
                ? "Networking"
                : res.resourceType.includes("instance") ||
                    res.resourceType.includes("cluster") ||
                    res.resourceType.includes("autoscaling")
                  ? "Compute"
                  : res.resourceType.includes("bucket") ||
                      res.resourceType.includes("volume")
                    ? "Storage"
                    : res.resourceType.includes("db_") ||
                        res.resourceType.includes("elasticache") ||
                        res.resourceType.includes("redis")
                      ? "Database"
                      : "Other";
            resources.push({
              id: res.id,
  ...
  const groupedResources = useMemo(() => {
    ...
    const groups = new Map<
      string,
      { provider: ProviderType; resources: ImportedResource[] }
    >();
    for (const res of selected) {
      const g = groups.get(res.groupName) || {
        provider: res.provider,
        resources: [],
      };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Importe recursos existentes da nuvem para gerar um diagrama no
                Canvas
              </p>
            </div>
          </div>
        ...
        >
          <div className="px-6 pt-4">
            <TabsList className="w-full bg-slate-100 h-auto p-1">
              <TabsTrigger
                value="provider-scan"
                className="flex-1 text-xs py-2 gap-1.5 data-[state=active]:shadow-sm"
              >
              ...
              {connectedCredentials.length === 0 &&
                ...
                !scanResult && (
                  <div className="flex items-center gap-2 px-4 py-3 bg-amber-50 border border-amber-100 rounded-xl text-xs text-amber-700">
                    ...
                    Nenhuma credencial conectada. Configure credenciais em
                    Configurações &rarr; Credenciais.
                  </div>
                )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-brand-navy">
                        Escaneando infraestrutura...
                      </p>
                      <p className="text-xs text-slate-400">
                        Descoberta de recursos via API do provedor
                      </p>
                    </div>
                  </div>
              ...
              {scanResult && !scanning && (
                <div className="space-y-3">
                      ...
                      <button
                        onClick={() => {
                          const allIds = new Set(
                            scanResult.flatMap((g) =>
                              g.resources.map((r) => r.id),
                            ),
                          );
                          setSelectedResourceIds(allIds);
                        }}
                  ...
                  {scanResult.map((group) => {
                    ...
                    const groupChecked = group.resources.every((r) =>
                    ...
                    return (
                      <div key={group.name} className="space-y-1">
                        <div className="flex items-center gap-2 px-1">
                          <button
                            onClick={() =>
                              toggleGroup(group.resources, !groupChecked)
                            }
                            className={cn(
                          ...
                          >
                            ...
                            {groupPartial && (
                              <div className="w-2 h-0.5 bg-brand-navy rounded" />
                            )}
                          </button>
                          ...
                          {group.resources.map((res) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                />
                                <span className="text-xs font-medium text-brand-navy truncate flex-1">
                                  {res.name}
                                </span>
                                <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-500 font-mono shrink-0">
                                  {getServiceDisplayType(res.resourceType)}
                                </span>
                              </div>
                            );
                          })}
                ...
                <>
                  ...
                  >
                    ...
                    <p className="text-sm font-medium text-brand-navy mb-1">
                      {stateFileName ||
                        "Clique para selecionar o arquivo .tfstate"}
                    </p>
                    <p className="text-xs text-slate-400">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-500">
                      ...
                      <span>
                        <strong className="text-brand-navy">
                          {stateParseResult.length}
                        </strong>{" "}
                        recursos encontrados no estado
                      </span>
                    </div>
                    <div className="flex gap-2">
                      <button
                      ...
                      <div key={g.name} className="space-y-1">
                        ...
                        <div className="space-y-0.5 ml-6">
                          {g.resources.map((res) => (
                            <div
                              key={res.id}
                              className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-transparent"
                            >
                              <span
                                className="w-1.5 h-1.5 rounded-full shrink-0"
                                style={{
                  ...
                  {tfFiles.map((file, i) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-slate-400">
                          {file.resources.length} recursos
                        </span>
                      </div>

      ...
      {reposWithApp.length === 0 && appDeploymentsForEnv.length === 0 && (
        <div className="py-6 text-center">
          ...
          <p className="text-xs text-slate-400">
            Nenhum repositório com aplicação detectada. Conecte um repositório
            no módulo Platform.
          </p>
        </div>
      )}
              ...
              >
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {dep.appType && (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold bg-ice-blue text-brand-navy border border-ice-blue">
                            {APP_TYPE_LABELS[
                              dep.appType as keyof typeof APP_TYPE_LABELS
                            ] || dep.appType}
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <span className="text-[10px] text-slate-400">
                          {DEPLOY_TARGET_LABELS[dep.targetType]}
                        </span>
                        <span className="text-[10px] text-slate-400">
                          {dep.version}
                        </span>
                    ...
                    <span
                      className={cn(
                        ...
                        dep.healthStatus === "healthy"
                          ...
                          : dep.healthStatus === "degraded"
                            ? "bg-amber-50 text-amber-700 border-amber-200"
                            : dep.healthStatus === "down"
                ...
                {dep.deployedAt && (
                  <p className="text-[10px] text-slate-400 mt-2">
                    Último deploy:{" "}
                    {new Date(dep.deployedAt).toLocaleString("pt-BR")}
                    {dep.lastHealthCheck &&
                      ` · Último health check: ${new Date(dep.lastHealthCheck).toLocaleString("pt-BR")}`}
                  </p>
                )}

...
export function ApprovalDialog({ onClose }: ApprovalDialogProps) {
  ...
  const { environments, deployments, addDeployment, updateEnvironment } =
    useCredentialStore();
  ...
  const selectedHistory = useMemo(
    () =>
      selectedId
        ? approvalHistory.filter((h) => h.approvalRequestId === selectedId)
        : [],
  ...
  const relatedPromotion = useMemo(
    () =>
      selectedRequest
        ? promotions.find((p) => p.id === selectedRequest.promotionId)
        : null,
  ...
  const handleApprove = async () => {
    ...
    const sourceEnv = environments.find(
      (e) => e.id === selectedRequest.sourceEnvId,
    );
    const targetEnv = environments.find(
      (e) => e.id === selectedRequest.targetEnvId,
    );
    ...
    if (targetEnv) {
      addDeployment({
        ...
        planSummary: {
          add: selectedRequest.resourceCount,
          change: 0,
          destroy: 0,
        },
      });
              ...
              {pendingRequests.length === 0 ? (
                ...
                <div className="space-y-3">
                  {pendingRequests.map((req) => {
                    const sourceEnv = environments.find(
                      (e) => e.id === req.sourceEnvId,
                    );
                    const targetEnv = environments.find(
                      (e) => e.id === req.targetEnvId,
                    );
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-1.5 mt-0.5">
                                ...
                                >
                                  {ENVIRONMENT_TYPE_LABELS[req.targetEnvType] ??
                                    req.targetEnvType}
                                </span>
                              </div>
                            </div>
                ...
                <div className="space-y-2">
                  {historyEntries.map((entry) => {
                    const req = approvalRequests.find(
                      (r) => r.id === entry.approvalRequestId,
                    );
                    return (
                      <div
                        key={entry.id}

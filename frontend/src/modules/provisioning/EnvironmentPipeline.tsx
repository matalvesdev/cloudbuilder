...
import { ArrowRight, CheckCircle2, Loader2, XCircle, Clock, AlertTriangle, ArrowUpFromLine } from 'lucide-react'
...
export function EnvironmentPipeline({ onPromote, environments }: EnvironmentPipelineProps) {
  const deployments = useCredentialStore((s) => s.deployments)
  const promotions = usePromotionStore((s) => s.promotions)
  const nodes = useCanvasStore((s) => s.nodes)

  const sorted = useMemo(
    () => [...environments].sort((a, b) => (ENV_ORDER[a.type] ?? 99) - (ENV_ORDER[b.type] ?? 99)),
    [environments]
  )
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-card mb-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[12px] font-bold uppercase tracking-wide text-slate-400">Pipeline de Ambientes</h3>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2">
        {sorted.map((env, idx) => {
          ...
          const pendingPromotions = envPromotions.filter((p) => p.status === 'pending')
          const failedPromotions = envPromotions.filter((p) => p.status === 'failed')

          const nextType = getNextEnvType(env.type)
          const nextEnv = nextType ? sorted.find((e) => e.type === nextType) : null
            ...
            <div key={env.id} className="flex items-center gap-2 shrink-0">
              <div
                className={cn(
                  ...
                  env.status === 'ACTIVE' ? 'border-green-300 bg-green-50/50' :
                  env.status === 'FAILED' ? 'border-red-300 bg-red-50/50' :
                  'border-slate-200 bg-white'
                )}
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold text-brand-navy">{env.name}</span>
                  <span className={cn(
                ...
                <div className="flex items-center gap-2 text-xs text-slate-500 mb-2">
                  <span className="font-mono font-bold text-brand-navy">v{env.canvasVersion}</span>
                  {latestDeploy && (
                    <span className={cn(
                      'inline-flex items-center gap-0.5 text-[10px] font-semibold',
                      latestDeploy.status === 'success' ? 'text-green-600' :
                      latestDeploy.status === 'failed' ? 'text-red-600' :
                      'text-amber-600'
                    )}>
                      {latestDeploy.status === 'success' ? <CheckCircle2 className="w-3 h-3" /> :
                       latestDeploy.status === 'failed' ? <XCircle className="w-3 h-3" /> :
                       <Clock className="w-3 h-3" />}
                      {latestDeploy.status === 'success' ? 'Deploy OK' :
                       latestDeploy.status === 'failed' ? 'Falhou' :
                       'Em andamento'}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex -space-x-1">
                    {[1, 2, 3].slice(0, Math.min(env.canvasVersion, 3)).map((v) => (
                      <div
                        key={v}
                  ...
                  {nextEnv && (
                    <button
                      ...
                      disabled={!latestDeploy || latestDeploy.status !== 'success'}
                      className={cn(
                  ...
                  {!nextEnv && env.type === 'production' && (
                    <span className="ml-auto text-[10px] text-slate-400 italic">Fim da pipeline</span>
                  )}
                </div>

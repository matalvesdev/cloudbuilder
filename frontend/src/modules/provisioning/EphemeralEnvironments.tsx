    ...
    >
      ...
      <div className="flex items-center gap-4 text-[11px]">
        ...
        <span className="inline-flex items-center gap-1 text-slate-500">
          <DollarSign className="w-3 h-3" />$ {env.cost.toFixed(2)}
        </span>
      </div>
      ...
      {env.resources.length > 0 && (
        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-100">
          {env.resources.map((r, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-medium bg-slate-100 text-slate-600 border border-slate-200"
            >
      ...
      {loading && environments.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm font-semibold text-brand-navy mb-1">
            Nenhum ambiente efêmero
          </p>
          <p className="text-[11px] text-slate-400 max-w-sm">
            Crie ambientes temporários para testar PRs e branches de feature com
            um custo reduzido.
          </p>
        </div>
      ) : (

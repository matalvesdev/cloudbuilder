...
const METRIC_CARDS = [
  { title: 'CPU', icon: Cpu, metric: 'cloudbuilder.api.metrics.duration', suffix: '%' },
  { title: 'Memória', icon: HardDrive, metric: 'cloudbuilder.api.metrics.count', suffix: '%' },
  { title: 'Rede', icon: Wifi, metric: 'cloudbuilder.api.traces.count', suffix: ' req/s' },
  { title: 'Latência', icon: Activity, metric: 'cloudbuilder.api.metrics.duration', suffix: 'ms' },
]

export function MetricsDashboard() {
  const [timeRange, setTimeRange] = useState<TimeRange>('1h')
  const { data, connected } = useSSE<MetricQueryResult[]>('/observability/metrics/stream', 'metrics')
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-brand-navy font-display">Métricas em Tempo Real</h2>
          {connected && (
      ...
      <div className="grid grid-cols-4 gap-4">
        {METRIC_CARDS.map(({ title, icon: Icon, suffix }) => (
          <div key={title} className="p-4 bg-white rounded-3xl card-shadow border border-slate-100">
      ...
      <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-6">
        <h3 className="text-sm font-bold text-brand-navy font-display mb-4">Tendência</h3>
        {chartData.length === 0 ? (

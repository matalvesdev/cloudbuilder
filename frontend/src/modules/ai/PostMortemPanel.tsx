...
function PriorityBadge({ priority }: { priority: ActionItem['priority'] }) {
  const config = {
    ...
    medium: { bg: 'bg-amber-50 text-amber-700 border-amber-200', label: 'Média' },
...
function ActionItemStatusBadge({ status }: { status: ActionItem['status'] }) {
  const config = {
    'open': { bg: 'bg-slate-50 text-slate-600 border-slate-200', label: 'Aberto' },
    'in-progress': { bg: 'bg-blue-50 text-blue-700 border-blue-200', label: 'Em Andamento' },
    'completed': { bg: 'bg-green-50 text-green-700 border-green-200', label: 'Concluído' },
  }[status]
...
export function PostMortemPanel({
  ...
  const [internalReport, setInternalReport] = useState<PostMortemReport | null>(null)
  ...
  const filteredActionItems = report?.actionItems.filter((ai) => {
    ...
    return (
      <div className={cn('space-y-4', className)}>
        <div className="bg-white border border-slate-200 rounded-xl p-8 text-center">
          ...
          <h3 className="text-sm font-bold text-brand-navy mb-1">Relatório Pós-Mortem</h3>
          <p className="text-xs text-slate-400 mb-4 max-w-md mx-auto">
            Gere um relatório pós-mortem baseado nos incidentes recentes. O relatório inclui timeline,
            causa raiz, impacto, ações corretivas e lições aprendidas.
          </p>
          <button
            ...
            className={cn(
              ...
              generating ? 'bg-slate-400 cursor-not-allowed' : 'bg-brand-navy hover:bg-brand-navy/90'
            )}
          >
            {generating ? (
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className={cn(
            ...
            report.status === 'final' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-amber-50 text-amber-700 border-amber-200'
          )}>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <h4 className="text-[11px] font-bold text-brand-navy uppercase tracking-wide mb-2">Resumo</h4>
        <p className="text-xs text-slate-600 leading-relaxed">{report.summary}</p>
      </div>

      {/* Timeline */}
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <h4 className="text-[11px] font-bold text-brand-navy uppercase tracking-wide mb-3">Timeline do Incidente</h4>
            ...
            <div key={idx} className="relative flex gap-3 pb-4 last:pb-0">
              ...
              <div className="relative mt-0.5 shrink-0">
                <div className={cn(
                  ...
                  idx === 0 ? 'bg-red-100' : idx === report.timeline.length - 1 ? 'bg-green-100' : 'bg-ice-blue'
                )}>
                  <Clock className={cn(
                    'w-2.5 h-2.5',
                    idx === 0 ? 'text-red-500' : idx === report.timeline.length - 1 ? 'text-green-600' : 'text-brand-navy'
                  )} />
                </div>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold text-brand-navy">{entry.event}</span>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <h4 className="text-[11px] font-bold text-brand-navy uppercase tracking-wide mb-2">Causa Raiz</h4>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <h4 className="text-[11px] font-bold text-brand-navy uppercase tracking-wide mb-2">Impacto</h4>
          <div className="text-xs text-slate-600 leading-relaxed prose prose-xs max-w-none">
            {report.impact.split('\n').map((line, i) => (
              <p key={i}>{line}</p>
            ))}
      ...
      {/* Resolution */}
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <h4 className="text-[11px] font-bold text-brand-navy uppercase tracking-wide mb-2">Resolução</h4>
        <div className="text-xs text-slate-600 leading-relaxed prose prose-xs max-w-none">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-[11px] font-bold text-brand-navy uppercase tracking-wide">Itens de Ação</h4>
          <select
            value={statusFilter}
        ...
        <div className="space-y-2">
          {filteredActionItems.length === 0 ? (
            <p className="text-xs text-slate-400 text-center py-4">Nenhum item de ação encontrado</p>
          ) : (
            filteredActionItems.map((item) => (
              <div key={item.id} className="flex items-start gap-3 p-3 rounded-lg border border-slate-100 hover:bg-slate-50 transition-colors">
                <div className={cn(
                  ...
                  item.status === 'completed' ? 'bg-green-50 border-green-300' : 'border-slate-300'
                )}>
                  {item.status === 'completed' && <CheckCircle2 className="w-3 h-3 text-green-500" />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-brand-navy">{item.description}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] text-slate-400">Responsável: {item.owner}</span>
                    <PriorityBadge priority={item.priority} />
                    <ActionItemStatusBadge status={item.status} />
                    <span className="text-[10px] text-slate-400">Vencimento: {item.dueDate}</span>
                  </div>
                </div>
              </div>
            ))
      ...
      {/* Lessons Learned */}
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <h4 className="text-[11px] font-bold text-brand-navy uppercase tracking-wide mb-3">Lições Aprendidas</h4>
        <div className="space-y-2">

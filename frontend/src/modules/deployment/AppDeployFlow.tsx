...
import {
  ArrowLeft, ArrowRight, CheckCircle2, Loader2, XCircle, Globe, GitBranch,
  FileCode2, ExternalLink, Server, Box, Layers, Cpu, Github, GitFork,
} from 'lucide-react'
...
import {
  APP_TYPE_LABELS, DEPLOY_TARGET_LABELS, CI_PROVIDER_LABELS,
  type AppType, type DeployTargetType, type CiProvider,
...
function detectDeployTarget(appType: AppType, provider: string): DeployTargetType {
  if (appType === 'data-pipeline') return 'lambda'
  if (appType === 'docker-compose' || appType === 'microservice') return 'ecs_service'
...
export function AppDeployFlow({ environmentId, infraStackId, onClose, onDeployed }: AppDeployFlowProps) {
  const [step, setStep] = useState<Step>('select-app')
  const [selectedRepoId, setSelectedRepoId] = useState<string>('')
  const [selectedTarget, setSelectedTarget] = useState<DeployTargetType>('ecs_service')
  ...
  const selectedDetection = selectedRepoId ? detectAppType(selectedRepoId) : null
  const selectedScan = selectedRepoId ? scanResults.find((r) => r.repoId === selectedRepoId) : null
  ...
  const handleNext = () => {
    if (step === 'select-app' && selectedRepoId) {
      ...
      if (repo) {
        ...
        setSelectedTarget(detectDeployTarget(detectedAppType, environment?.provider || 'aws'))
      }
      setStep('configure')
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      const pipelineYaml = generatePipelineYaml(ciProvider, selectedRepo, detectedAppType, selectedTarget)
      ...
      const pipelineYamlStr = generatePipelineYaml(ciProvider, selectedRepo, detectedAppType, selectedTarget)
      ...
      >
        ...
        <div className="p-6 border-b border-slate-100">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-bold text-brand-navy font-display">Deploy de Aplicação</h2>
            {environment && (
              ...
              return (
                <div key={i} className="flex items-center gap-2 flex-1">
                  <div className={cn(
                    ...
                    idx === current ? 'bg-brand-navy text-white' :
                    'bg-slate-100 text-slate-400'
                  )}>
                    {idx < current ? <CheckCircle2 className="w-3.5 h-3.5" /> : idx}
                  </div>
                  <span className={cn(
        ...
        <div className="p-6 space-y-5">
          ...
          {step === 'select-app' && (
            <>
              <p className="text-sm text-slate-500">
                Selecione o repositório da aplicação que deseja implantar na infraestrutura provisionada.
              </p>

              {reposWithApp.length === 0 ? (
                <div className="py-10 text-center">
                  <Box className="w-10 h-10 text-slate-300 mx-auto mb-3" />
                  <p className="text-sm font-medium text-slate-500">Nenhum repositório com aplicação detectada</p>
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            <div className={cn(
                              ...
                              repo.provider === 'gitlab' ? 'bg-orange-500' :
                              'bg-blue-600'
                            )}>
                              {repo.provider === 'github' ? 'GH' : repo.provider === 'gitlab' ? 'GL' : 'BB'}
                            </div>
                            <div>
                              <p className="text-sm font-bold text-brand-navy">{repo.fullName}</p>
                              <div className="flex items-center gap-2 mt-0.5">
                                {appType && (
                                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-ice-blue text-brand-navy">
                                    {APP_TYPE_LABELS[appType as keyof typeof APP_TYPE_LABELS] || appType}
                                  </span>
                                )}
                                {scan?.languages?.length && (
                                  <span className="text-[10px] text-slate-400">{scan.languages.join(', ')}</span>
                                )}
                              </div>
                            </div>
                          </div>
                        ...
                        {detection?.framework && (
                          <p className="text-[11px] text-slate-400 mt-2 ml-12">
                            Framework: <span className="font-medium text-slate-500">{detection.framework}</span>
                            {detection.hasDockerfile && ' · Dockerfile detectado'}
                            {detection.hasK8sManifest && ' · Manifesto K8s detectado'}
                          </p>
                        )}
                      </button>
                    )
          ...
          {step === 'configure' && selectedRepo && (
            <>
              <p className="text-sm text-slate-500">
                Configure como a aplicação <strong>{selectedRepo.fullName}</strong> será implantada na infraestrutura.
              </p>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(['web-app', 'microservice', 'data-pipeline', 'docker-compose'] as const).map((type) => (
                      <button
                        key={type}
                        onClick={() => {}}
                        className={cn(
                        ...
                        disabled
                      >
                        {type === 'web-app' ? <Globe className="w-4 h-4" /> :
                         type === 'microservice' ? <Box className="w-4 h-4" /> :
                         type === 'data-pipeline' ? <Layers className="w-4 h-4" /> :
                  ...
                  {detectedAppType && (
                    <p className="text-xs text-slate-400 mt-1.5">
                      Tipo detectado automaticamente a partir do código no repositório
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(['ecs_service', 'lambda', 'k8s_deployment', 'ec2'] as const).map((target) => {
                      const isRecommended = target === detectDeployTarget(detectedAppType, environment?.provider || 'aws')
                      return (
                        <button
                          key={target}
                        ...
                        >
                          {target === 'ecs_service' ? <Server className="w-4 h-4" /> :
                           target === 'lambda' ? <Cpu className="w-4 h-4" /> :
                           target === 'k8s_deployment' ? <Box className="w-4 h-4" /> :
            ...
            <>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      ciProvider === 'gitlab_ci'
                        ? 'border-brand-navy bg-brand-navy/5 text-brand-navy'
                        : 'border-slate-200 text-slate-500 hover:border-slate-300',
                      selectedRepo.provider === 'bitbucket' && 'opacity-40 cursor-not-allowed'
                    )}
                  >
              ...
              <div>
                ...
                <div className="relative group">
                  <button
                    onClick={async () => {
                      const yaml = generatePipelineYaml(ciProvider, selectedRepo, detectedAppType, selectedTarget)
                  ...
                  <pre className="bg-[#0D1B2A] rounded-xl p-4 font-mono text-[10px] leading-relaxed max-h-[280px] overflow-y-auto text-slate-300 whitespace-pre scrollbar-thin border border-slate-700">
                    {generatePipelineYaml(ciProvider, selectedRepo, detectedAppType, selectedTarget)}
                  </pre>
                </div>
              </div>
              ...
              {selectedScan && (
                <div className="flex items-center gap-3 p-3 rounded-xl bg-ice-blue/30 border border-ice-blue">
                  ...
                  <p className="text-xs text-brand-navy">
                    Pipeline será configurado na branch <strong>{selectedRepo.defaultBranch}</strong> de <strong>{selectedRepo.fullName}</strong>
                  </p>
                </div>
              )}
            </>
          ...
          {/* ─── Step 4: Review ─── */}
          {step === 'review' && selectedRepo && selectedDetection && environment && (
            <>
              <p className="text-sm text-slate-500">
              ...
              <div className="bg-slate-50 rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold text-brand-navy">{appName}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500">Repositório</span>
                  <span className="font-semibold text-brand-navy">{selectedRepo.fullName}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500">Tipo</span>
                  <span className="font-semibold text-brand-navy">
                    {detectedAppType ? APP_TYPE_LABELS[detectedAppType as keyof typeof APP_TYPE_LABELS] : '—'}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500">Alvo</span>
                  <span className="font-semibold text-brand-navy">{DEPLOY_TARGET_LABELS[selectedTarget]}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500">Ambiente</span>
                  <span className="font-semibold text-brand-navy">{environment.name}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500">CI Provider</span>
                  <span className="font-semibold text-brand-navy">{CI_PROVIDER_LABELS[ciProvider]}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500">Branch</span>
                  <span className="font-semibold text-brand-navy">{selectedRepo.defaultBranch}</span>
                </div>
              </div>
              ...
              {deployDone && (
                <div className="space-y-3">
                  <div className="flex items-center gap-3 p-4 rounded-xl bg-green-50 border border-green-200">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-green-700">Deploy realizado com sucesso!</p>
                      <p className="text-xs text-green-500">Aplicação implantada e saudável</p>
                    </div>
                  </div>
                  {appUrl && (
                    <a
              ...
              {deploying && (
                <div className="flex items-center gap-3 p-4 rounded-xl bg-blue-50 border border-blue-200">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-blue-700">Implantando aplicação...</p>
                    <p className="text-xs text-blue-500">Configurando infraestrutura e fazendo deploy</p>
                  </div>
                </div>
              )}
            </>

...
function getResourceIcon(resourceType?: string) {
  ...
  if (t.includes('rds') || t.includes('db') || t.includes('dynamodb')) return <Database className="w-4 h-4" />
  if (t.includes('ecs') || t.includes('lambda') || t.includes('ec2')) return <Server className="w-4 h-4" />
  if (t.includes('s3') || t.includes('bucket') || t.includes('volume')) return <HardDrive className="w-4 h-4" />
  if (t.includes('sg') || t.includes('security') || t.includes('waf') || t.includes('cognito')) return <Shield className="w-4 h-4" />
  if (t.includes('vpc') || t.includes('subnet') || t.includes('igw') || t.includes('alb') || t.includes('nlb')) return <Network className="w-4 h-4" />
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Revise as alterações antes de aplicar a correção para: <strong>{incidentTitle}</strong>
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-brand-lime/10 border border-brand-lime/30 rounded-lg p-3 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-brand-navy">Correção Sugerida</p>
          ...
          <div>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Alterações por Recurso</p>
                ...
                {currentDesignNodes.map((node, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 border-b border-slate-200">
                      <div className={cn('w-1.5 h-6 rounded-full shrink-0', PROVIDER_COLORS[node.provider])}
                        style={{
                          backgroundColor:
                            node.provider === 'aws' ? '#FF9900' :
                            node.provider === 'azure' ? '#0078D4' :
                            node.provider === 'gcp' ? '#4285F4' : '#326CE5',
                        }}
                      />
                      {getResourceIcon(node.resourceType)}
                      <span className="text-xs font-bold text-brand-navy">{node.label}</span>
                      <span className="text-[10px] text-slate-400 font-mono ml-auto">{node.property}</span>
                    </div>
                    <div className="grid grid-cols-[1fr_auto_1fr] gap-2 p-3 items-center">
                      {/* Current */}
                      <div className="bg-red-50 border border-red-200 rounded-lg p-2.5">
                        <p className="text-[10px] font-bold text-red-600 uppercase mb-0.5">Atual</p>
                      ...
                      <div className="bg-green-50 border border-green-200 rounded-lg p-2.5">
                        <p className="text-[10px] font-bold text-green-600 uppercase mb-0.5">Corrigido</p>
          ...
          <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200">
            ...
            <p className="text-xs text-amber-800">
              Esta correção será aplicada ao design atual no canvas. As alterações podem ser desfeitas via undo.
              {modifications.length === 0 && ' Nenhuma modificação detectada no canvas atual.'}
            </p>
          </div>
        </div>

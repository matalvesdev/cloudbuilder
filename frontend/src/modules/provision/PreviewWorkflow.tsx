...
const actionConfig = {
  add: { icon: Plus, color: 'text-green-600', bg: 'bg-green-50 border-green-200', label: 'Adicionar' },
  change: { icon: Pencil, color: 'text-blue-600', bg: 'bg-blue-50 border-blue-200', label: 'Modificar' },
...
export function PreviewWorkflow({ planResult, planning, onPlan, onApply, hasCanvas, hasEnvironment, isDeployed }: PreviewWorkflowProps) {
  ...
  const hasBreakdown = planResult && planResult.resources && planResult.resources.length > 0
    ...
    return (
      <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-brand-navy">Preview do Deploy</h3>
            <p className="text-xs text-slate-400">Visualize as mudanças antes de aplicar</p>
          </div>
        </div>
        {!hasCanvas && (
    ...
    <div className="bg-white rounded-3xl card-shadow border border-slate-100 p-6 space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h3 className="text-sm font-bold text-brand-navy">Preview do Deploy</h3>
          <p className="text-xs text-slate-400">Revise as mudanças antes de aplicar</p>
        </div>
        {planResult && (
          <div className="flex items-center gap-2">
            {planResult.add > 0 && <Badge variant="default" className="bg-green-100 text-green-700 hover:bg-green-100 border-0 gap-1"><Plus className="w-3 h-3" />{planResult.add}</Badge>}
            {planResult.change > 0 && <Badge variant="default" className="bg-blue-100 text-blue-700 hover:bg-blue-100 border-0 gap-1"><Pencil className="w-3 h-3" />{planResult.change}</Badge>}
            {planResult.destroy > 0 && <Badge variant="destructive" className="gap-1"><Minus className="w-3 h-3" />{planResult.destroy}</Badge>}
          </div>
        )}
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-xl bg-green-50 border border-green-200 p-3 text-center">
              ...
              <p className="text-lg font-bold text-green-700">{planResult.add}</p>
              <p className="text-[10px] text-green-600 font-medium">Adicionar</p>
            </div>
            <div className="rounded-xl bg-blue-50 border border-blue-200 p-3 text-center">
              <Pencil className="w-4 h-4 text-blue-600 mx-auto mb-1" />
              <p className="text-lg font-bold text-blue-700">{planResult.change}</p>
            ...
            <div className="rounded-xl bg-red-50 border border-red-200 p-3 text-center">
              ...
              <p className="text-lg font-bold text-red-700">{planResult.destroy}</p>
          ...
          {hasBreakdown && (
            <div>
              ...
              >
                ...
                <ArrowRight className={cn('w-3.5 h-3.5 transition-transform', showDetails && 'rotate-90')} />
              </button>
              {showDetails && (
                <div className="space-y-1 mt-2 max-h-[200px] overflow-y-auto">
                  {planResult.resources.map((r, i) => {
                    ...
                    const Icon = cfg.icon
                    return (
                      <div key={i} className={cn('flex items-center gap-3 rounded-xl border p-3', cfg.bg)}>
                        <Icon className={cn('w-4 h-4 shrink-0', cfg.color)} />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold text-brand-navy truncate">{r.name}</p>
                          <p className="text-[10px] text-slate-500">{r.resourceType}</p>
                        </div>
          ...
          <div className="flex gap-3 pt-1">
            ...
            >
              <Loader2 className={cn('w-3.5 h-3.5', planning && 'animate-spin')} />
              Regenerar
            </button>
            <button
              onClick={onApply}

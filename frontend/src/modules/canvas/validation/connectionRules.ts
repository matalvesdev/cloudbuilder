...
const connectionRules: ConnectionRule[] = [
  ...
  },
  {
    ...
    check: (source, target) => {
      if (
        source.data.resourceType === "aws_subnet" &&
        target.data.resourceType === "aws_instance"
      ) {
        return { valid: true };
      }
  ...
  },
  {
    ...
    check: (source, target) => {
      if (
        source.data.resourceType === "aws_security_group" &&
        target.data.resourceType === "aws_instance"
      ) {
        return { valid: true };
      }
  ...
  },
  {
    ...
    check: (source, target) => {
      if (
        source.data.resourceType === "aws_vpc" &&
        target.data.resourceType === "aws_subnet"
      ) {
        return { valid: true };
      }
      return { valid: true };
  ...
  },
  {
    ...
    check: (source, target) => {
      if (
        source.data.resourceType === "aws_lb" &&
        target.data.resourceType === "aws_instance"
      ) {
        return { valid: true };
      }

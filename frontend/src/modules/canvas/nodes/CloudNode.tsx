...
function CloudNode(props: CanvasNode) {
  ...
  const commentCount = useCollaborationStore(
    (s) => s.comments.filter((c) => c.nodeId === id && !c.resolved).length,
  );
  const setSelectedCommentNodeId = useCollaborationStore(
    (s) => s.setSelectedCommentNodeId,
  );
  const highlightedIncidentNodes = useCanvasStore(
    (s) => s.highlightedIncidentNodes,
  );
  ...
  const estimatedCost = useMemo(
    () => getResourcePrice(data.resourceType || ""),
    [data.resourceType],
  );
  ...
  return (
    <div
      className={cn(
        ...
        selected
          ? `${theme.border} ring-2 ring-brand-lime/40 shadow-md`
          : "border-slate-200 hover:border-slate-300",
        isLocked && "opacity-85",
        isIncidentHighlighted &&
          "border-red-400 ring-2 ring-red-300 animate-pulse shadow-lg shadow-red-200/50",
      )}
    >
      ...
      <Handle
        ...
        className={cn("w-3 h-3 !border-2 !border-white", theme.handle)}
      />
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            {isEditing ? (
              ...
              <>
                <div className="text-[13px] font-semibold text-brand-navy truncate leading-tight">
                  {data.label}
                </div>
                {isLocked && (
      ...
      {properties.length > 0 && (
        <div
          className={cn("mx-3 mb-2 px-2 py-1.5 rounded-lg", theme.propertyBg)}
        >
      ...
      <div className="px-3 pb-2.5">
        <span
          className={cn(
            "inline-flex items-center px-2 py-0.5 rounded text-[8px] font-mono font-bold",
            theme.badgeBg,
            theme.badgeText,
          )}
        >

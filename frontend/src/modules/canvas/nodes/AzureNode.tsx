...
function AzureNode(props: NodeProps<CanvasNode>) {
  const { data, selected } = props
  return (
    <div className={`rounded-xl border-2 bg-white card-shadow w-64 ${selected ? 'border-[#0078D4] ring-2 ring-brand-lime/40' : 'border-slate-100'}`}>
      <CanvasNodeToolbar {...props} />
      <Handle type="target" position={Position.Top} className="w-3 h-3 !bg-[#0078D4]" />
      <div className="bg-[#0078D4] text-white px-3 py-2 rounded-t-xl flex items-center gap-2">
        <ServiceIcon componentId={data.componentDefinitionId} size={18} />
        <span className="text-xs font-bold tracking-wider">Azure</span>
      ...
      <Handle type="source" position={Position.Bottom} className="w-3 h-3 !bg-[#0078D4]" />

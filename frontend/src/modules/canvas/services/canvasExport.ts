...
export function exportCanvas(): CanvasExportData | null {
  const { nodes, edges, canvasId, canvasName, canvasVersion } =
...
export function exportCanvasV2(): CanvasExportDataV2 {
  ...
  const providers = Array.from(
    new Set(state.nodes.map((n) => n.data.provider)),
  );
  const providerType =
    providers.length === 1
      ? (providers[0] as "aws" | "azure" | "gcp" | "k8s")
      : "multi";
  ...
  return {
    ...
    connections: state.edges.map((e) => {
      ...
      return {
        source: sourceNode
          ? `${sourceNode.data.resourceType}.${sourceNode.data.label}`
          : e.source,
        target: targetNode
          ? `${targetNode.data.resourceType}.${targetNode.data.label}`
          : e.target,
        type: "reference",
      };
    }),
...
export async function importCanvasFromFile(file: File): Promise<void> {
  ...
  if (!isV2 && !isV1) {
    throw new Error(
      "Formato não reconhecido. Use .cloudbuilder.json (exportado do CloudBuilder).",
    );
  }

  const { loadCanvas, addNode, addEdgeWithType, autoLayout } =
    useCanvasStore.getState();

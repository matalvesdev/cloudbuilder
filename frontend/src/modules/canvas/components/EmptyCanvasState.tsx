import { Box, LayoutGrid, Network, Database, Sparkles, ArrowRight } from 'lucide-react'
...
const TEMPLATES: Template[] = [
  ...
  },
  {
    ...
    icon: Database,
    components: ['aws-vpc', 'aws-subnet', 'aws-subnet', 'aws-rds', 'aws-elasticache', 'aws-sg'],
...
export function EmptyCanvasState() {
  ...
  const loadTemplate = async (template: Template) => {
    ...
    for (const compId of template.components) {
      ...
      if (!comp) continue
      addNode(comp, { x: 150 + Math.random() * 400, y: 100 + Math.random() * 300 })
      ...
      <div className="pointer-events-auto max-w-xl w-full mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 max-w-sm mx-auto leading-relaxed">
            Arraste componentes da paleta ao lado ou escolha um template para começar.
          </p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="font-bold text-sm text-brand-navy">{template.title}</span>
                </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-xs text-slate-400">
            Ou use{' '}
            <kbd className="px-1.5 py-0.5 bg-slate-100 rounded text-[10px] font-mono text-slate-500 border border-slate-200">⌘K</kbd>

...
const TEMPLATES: Record<
  string,
  { label: string; components: string[]; edges?: [string, string][] }
> = {
  "vpc-basico": {
    label: "VPC Básica",
  ...
  "azure-infra": {
    ...
    edges: [
      ...
      ["azure-vnet", "azure-nsg"],
      ["azure-appgw", "azure-vm"],
      ["azure-vm", "azure-sql"],
  ...
  "k8s-production": {
    ...
    edges: [
      ["k8s-namespace", "k8s-deploy"],
      ["k8s-namespace", "k8s-configmap"],
      ["k8s-namespace", "k8s-secret"],
      ["k8s-deploy", "k8s-service"],
...
const WELCOME_MESSAGE: Message = {
  ...
  role: "assistant",
  content:
    "Oi! Posso ajudar a criar sua arquitetura na nuvem. Escolha um template ou descreva o que precisa.",
  suggestions: [
    "VPC básica",
    "Web App 3 camadas",
    "API Serverless",
...
function findComponents(
  ...
  for (const [category, aliases] of Object.entries(CATEGORY_ALIASES)) {
    if (aliases.some((a) => q.includes(a))) {
      const categoryMatches = allComponents.filter(
...
function detectIntent(
  input: string,
): "template" | "add" | "layout" | "clear" | "unknown" {
  const lower = input.toLowerCase();
  if (TEMPLATE_KEYWORDS.some(([_, kw]) => lower.includes(kw)))
    return "template";
  if (
...
export function AIChatPanel({ onClose }: { onClose?: () => void }) {
  ...
  const addSingleComponent = useCallback(
    async (query: string) => {
      ...
      appendAssistant(`Adicionei ${matches.length} componente(s) ao canvas.`, [
        "Organizar layout",
        "Adicionar mais",
      ]);
    },
    ...
    try {
      ...
      switch (intent) {
        case "template": {
          ...
          const matched = TEMPLATE_KEYWORDS.find(([_, kw]) =>
            lower.includes(kw),
          );
        ...
        case "add": {
          ...
          const query = userText
            .replace(
              /^(adicione|adicionar|add|colocar|criar|um|uma|dois|duas|três)\s+/i,
              "",
            )
            .trim();
        ...
        case "layout": {
          autoLayout();
          appendAssistant("Layout organizado automaticamente! ✅", [
            "Exportar código",
            "Salvar design",
          ]);
          break;
        ...
        case "clear": {
          clearCanvas();
          appendAssistant("Canvas limpo! Pronto para um novo design.", [
            "VPC básica",
            "Web App 3 camadas",
            "Microserviços AWS",
            "Infra Azure",
  ...
  const handleSuggestion = useCallback(
    async (suggestion: string) => {
      ...
      const templateMap: Record<
        string,
        { key: string; msg: string; suggestions: string[] }
      > = {
        "VPC básica": {
          key: "vpc-basico",
          msg: "Pronto! VPC básica criada com VPC, Subnets, Internet Gateway e Route Table.",
          suggestions: ["Organizar layout", "Adicionar EC2"],
        },
        "Web App 3 camadas": {
          key: "web-app-3-tier",
          msg: "Arquitetura 3 camadas criada! ALB → EC2 → RDS com Security Groups e S3.",
          suggestions: ["Organizar layout", "Exportar código"],
        },
        "API Serverless": {
          key: "serverless-api",
          msg: "API Serverless criada! Lambda → DynamoDB + SQS + SNS.",
          suggestions: ["Organizar layout", "Exportar código"],
        },
        "Microserviços AWS": {
          key: "microservices-aws",
          msg: "Microserviços AWS criados! ECS + ALB + RDS + ElastiCache + SQS + ECR.",
          suggestions: [
            "Organizar layout",
            "Exportar código",
            "Adicionar mais",
        ...
        "Kubernetes AWS (EKS)": {
          key: "k8s-eks",
          msg: "Cluster EKS criado! VPC + ECR + ALB + RDS + S3 + CloudWatch.",
          suggestions: ["Organizar layout", "Exportar código"],
        },
        "Infra Azure": {
          key: "azure-infra",
          msg: "Infraestrutura Azure criada! VNet + App Gateway + VM + SQL + Storage + Functions.",
          suggestions: ["Organizar layout", "Exportar código"],
        },
        "GCP Cloud Native": {
          key: "gcp-native",
          msg: "Arquitetura GCP criada! VPC + GKE + Cloud SQL + Cloud Storage + Cloud Run.",
          suggestions: ["Organizar layout", "Exportar código"],
        },
        "K8s Produção": {
          key: "k8s-production",
          msg: "Kubernetes Produção criado! Deployments + Services + Ingress + ConfigMaps + Secrets + PVC + HPA.",
          suggestions: ["Organizar layout", "Consolidar"],
        },
      };
      ...
      if (templateMatch) {
        ...
        appendAssistant(
          "Abra o painel de código abaixo (ícone </>) para ver o Terraform gerado.",
        );
        ...
        appendAssistant("O que mais deseja adicionar?", [
          "EC2",
          "RDS",
          "Lambda",
          "S3",
    ...
    <div className="w-96 bg-white border-l border-slate-100 flex flex-col shadow-xl">
      ...
      <div className="p-4 border-b border-slate-100 bg-white">
        ...
        <p className="text-xs text-slate-400 pl-9">
          Assistente de arquitetura inteligente
        </p>
      </div>

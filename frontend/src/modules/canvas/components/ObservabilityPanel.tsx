...
function incidentToAlert(incident: IncidentDTO): AlertDisplay {
  return {
    ...
    acknowledged:
      incident.acknowledgedAt !== null || incident.status === "RESOLVED",
  };
}
...
export function ObservabilityPanel({ onClose }: Props) {
  ...
  const fetchAlerts = useCallback(async () => {
    try {
      const incidents =
        (await observabilityApi.getActiveIncidents()) as IncidentDTO[];
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100">
        ...
        <span
          className={cn(
            ...
            !loadingAlerts ? "bg-green-400" : "bg-slate-300",
          )}
        />
        <button
          onClick={onClose}
      ...
      <div className="flex border-b border-slate-100">
        ...
        >
          ...
          {tab === "alerts" && (
            <div className="absolute bottom-0 left-2 right-2 h-0.5 bg-brand-navy rounded-full" />
          )}
        </button>
        <button
                ...
                alerts.map((alert) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() =>
                        !alert.acknowledged && handleAcknowledge(alert.id)
                      }
                      role={!alert.acknowledged ? "button" : undefined}
                    >
                      <div className="flex items-start gap-3">
                        <Icon
                          className={cn("w-4 h-4 mt-0.5 shrink-0", sev.color)}
                        />
      ...
      <div className="px-4 py-2 border-t border-slate-100 text-[10px] text-slate-400 flex items-center justify-between">
        ...
        {alerts.length > 0 && (
          <span>{fmtTime(Math.max(...alerts.map((a) => a.timestamp)))}</span>
        )}
      </div>
    </div>
  );

...
import { listRepos, listRepoContents, getFileContent } from "@/api/github";
...
export function RepoBrowser({ token, onClose }: RepoBrowserProps) {
  ...
  const loadFiles = async (repo: Repo, path: string) => {
    ...
    setError("");
    try {
      const data = await listRepoContents(repo.owner, repo.name, path, repo.defaultBranch);
      setFiles(data.files || []);
    } catch (err: any) {
  ...
  const handleImportFile = useCallback(
    async (file: FileItem) => {
      ...
      try {
        ...
        const isState =
          file.name.endsWith(".tfstate") || file.name.endsWith(".tfstate.json");
    ...
    try {
      for (const file of tfFiles) {
        ...
        const result = isState
          ? await importState(data.content)
          : await importTerraform(data.content);
        ...
        return (
          <div className="flex flex-col h-full">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100">
              ...
              <button
                onClick={onClose}
                className="ml-auto text-slate-400 hover:text-brand-navy transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              {pathParts.map((part, i) => (
                <span key={i} className="flex items-center gap-1">
                  ...
                  {i < pathParts.length - 1 ? (
                    <button
                      onClick={() =>
                        handleNavigateDir(pathParts.slice(0, i + 1).join("/"))
                      }
                      className="text-slate-500 hover:text-brand-navy"
                    >
                      {part}
            ...
            <div className="flex-1 overflow-y-auto">
              {error && (
                <div className="mx-4 mt-3 px-4 py-2 bg-red-50 border border-red-100 rounded-xl text-xs text-red-700">
                  {error}
                </div>
              )}
              {filesLoading ? (
  ...
  const repoListView = (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100">
        <Github className="w-4 h-4 text-slate-600" />
        <span className="text-sm font-bold text-brand-navy flex-1">
          Seus Repositórios
        </span>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="m-4 px-4 py-3 bg-red-50 border border-red-100 rounded-xl text-sm text-red-700">
            {error}
          </div>
          ...
          <div className="divide-y divide-slate-50">
            {filteredRepos.map((repo) => (
              ...
              >
                ...
                {repo.description && (
                  <p className="text-xs text-slate-400 ml-6 line-clamp-1">
                    {repo.description}
                  </p>
                )}

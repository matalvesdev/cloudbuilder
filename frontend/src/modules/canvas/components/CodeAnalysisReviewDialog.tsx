...
export function CodeAnalysisReviewDialog({
  open,
  onOpenChange,
  files,
  repoUrl,
  ...
  const handleApply = async () => {
    if (!result) return;
    const selectedResources = result.inferredResources.filter((_, i) =>
      selected.has(i),
    );
      ...
      <div className="w-[640px] max-h-[80vh] bg-white rounded-2xl shadow-modal flex flex-col">
          ...
          {!result && !analyzing && !error && (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-slate-500 mb-2">
                Analise os arquivos do projeto para detectar a stack técnica
              </p>
              <p className="text-xs text-slate-400 mb-6">
                O CloudBuilder vai identificar frameworks, bancos de dados e
          ...
          {analyzing && (
            <div className="text-center py-12">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Detectando frameworks e inferindo recursos
              </p>
            </div>
          )}
              ...
              <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
                ...
                <div>
                  <p className="text-sm font-bold text-brand-navy">
                    {result.detectedStack || "Stack personalizada"}
                  </p>
                  <p className="text-xs text-slate-500">
                    {result.stackDescription || "Stack não identificada"}
                  </p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {result.detectedFrameworks.map((fw) => (
                      <span
                        key={fw}
                        className="px-2.5 py-1 bg-brand-lime/15 text-brand-navy text-xs font-medium rounded-lg border border-brand-lime/30"
                      >
                        {fw}
                      </span>
                    ))}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">
                    ...
                    {result.inferredResources.length})
                  </p>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() =>
                        setSelected(
                          new Set(result.inferredResources.map((_, i) => i)),
                        )
                      }
                ...
                <div className="space-y-2">
                  {result.inferredResources.map(
                    (res: InferredResource, idx: number) => {
                        ...
                        >
                          ...
                          {isExpanded && (
                            <div className="px-4 pb-3 pt-0 border-t border-slate-50">
                              ...
                              {Object.keys(res.suggestedProperties).length >
                                0 && (
                                <div className="mt-2">
                                  ...
                                  <div className="flex flex-wrap gap-1.5">
                                    {Object.entries(
                                      res.suggestedProperties,
                                    ).map(([k, v]) => (
                                      <span
                                        key={k}
                                        className="px-2 py-0.5 bg-slate-50 rounded text-[10px] text-slate-500 font-mono border border-slate-100"
                                      >
                                        {k}: {String(v)}
                                      </span>
                                    ))}
                                ...
                                <div className="mt-2">
                                  ...
                                  <div className="flex flex-wrap gap-1.5">
                                    {res.evidence.map((e) => (
                                      <span
                                        key={e}
                                        className="px-2 py-0.5 bg-blue-50 rounded text-[10px] text-blue-600 font-mono border border-blue-100"
                                      >
                                        {e}
                                      </span>
                                    ))}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100">
          <p className="text-xs text-slate-400">
            {result
              ? `${selected.size} de ${result.inferredResources.length} recursos selecionados`
              : ""}
          </p>
          <div className="flex items-center gap-2">
            <button
            ...
            {result && (
              ...
              >
                <div className="flex items-center gap-1.5">
                  ...
                  {selected.size === 1 ? "recurso" : "recursos"} ao Canvas
                </div>
              </button>
            )}
          </div>

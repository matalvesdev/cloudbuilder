...
export function CanvasEventsPanel({ canvasId }: CanvasEventsPanelProps) {
  ...
  const typeColor = (type: string) => {
    if (type.includes('error') || type.includes('failed')) return 'bg-red-100 text-red-700'
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {filtered.length === 0 ? (
          <div className="text-slate-400 text-xs text-center py-4">Nenhum evento registrado</div>
        ) : (
          filtered.map(event => (
            <div key={event.id} className="flex items-start gap-2 p-2 rounded-lg hover:bg-slate-50">
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${typeColor(event.type)}`}>
                {event.type}
              </span>

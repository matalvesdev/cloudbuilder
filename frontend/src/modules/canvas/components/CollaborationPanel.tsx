...
function getAvatarColor(name: string): string {
  if (AVATAR_COLORS[name.charAt(0).toUpperCase()])
    return AVATAR_COLORS[name.charAt(0).toUpperCase()];
  let hash = 0;
...
function CompartilharSection() {
  const { teamMembers, inviteMember, removeMember, generateShareLink } =
  ...
  const handleToggleConnection = () => {
    if (connected) {
      ...
      collaborationManager.start(canvasId || "design-atual", serverUrl, {
        id: userId,
        name: userName,
        avatar: userInitial,
      }, user?.tenantId);
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  ROLE_BADGE[member.role].className,
                )}
              >
      ...
      <div className="space-y-3 flex-1">
        ...
        {activeNodeLabel && (
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-ice-blue rounded-lg">
            ...
            <span className="text-xs font-medium text-brand-navy truncate">
              {activeNodeLabel}
            </span>
            <button
              onClick={() => setActiveNodeId(null)}
      ...
      <div className="pt-3 border-t border-slate-100 mt-3">
        ...
        {!activeNodeId && (
          <p className="text-[9px] text-slate-400 mt-1.5">
            Selecione um nó para comentar sobre ele, ou comente aqui para um
            comentário geral.
          </p>
        )}
      </div>
      ...
      {versions.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-8 text-slate-400">
          ...
          <p className="text-[10px] text-slate-300">
            Salve o design para criar um snapshot
          </p>
        </div>
      ) : (
        <div className="space-y-1.5">
      ...
      <div className="px-4 py-3 border-b border-slate-100">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-bold text-sm text-brand-navy uppercase tracking-wider">
                Colaboração
              </h2>

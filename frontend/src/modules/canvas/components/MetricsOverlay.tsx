...
import {
  Activity, Cpu, MemoryStick, Network, HardDrive,
  X, AlertTriangle, AlertCircle, CheckCircle, Loader2,
} from 'lucide-react'
...
const statusConfig = {
  healthy: { icon: CheckCircle, color: 'text-green-500', bg: 'bg-green-50', border: 'border-green-200', label: 'Saudável' },
  ...
  unknown: { icon: Loader2, color: 'text-slate-400', bg: 'bg-slate-50', border: 'border-slate-200', label: 'Desconhecido' },
}
        ...
        <div className="absolute bottom-4 right-4 z-30 w-[300px] bg-white border border-slate-200 rounded-2xl shadow-modal overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-brand-navy">Métricas</span>
            </div>
            <div className="flex items-center gap-2">
              <span className={cn(
              ...
              <span className="text-[10px] text-slate-400">{connected ? 'Conectado' : 'Desconectado'}</span>
              <button onClick={onToggle} className="text-slate-300 hover:text-brand-navy ml-1">
          ...
          <div className="px-4 py-3 bg-slate-50 border-b border-slate-100">
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold text-brand-navy">{selectedMetrics.resourceName}</span>
              <span className={cn(
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-brand-navy">{fmtPct(latest(selectedMetrics.cpuUtilization))}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-brand-navy">{fmtPct(latest(selectedMetrics.memoryUtilization))}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-brand-navy">{fmtBytes(latest(selectedMetrics.networkIn))}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-brand-navy">{fmtBytes(latest(selectedMetrics.networkOut))}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-brand-navy">{fmtBytes(latest(selectedMetrics.diskReadOps))}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-brand-navy">{fmtBytes(latest(selectedMetrics.diskWriteOps))}</span>
            </div>
          </div>

          {/* Last updated */}
          <div className="px-4 py-2 border-t border-slate-100 text-[10px] text-slate-400 text-right">
            {lastUpdate ? `Atualizado ${new Date(lastUpdate).toLocaleTimeString('pt-BR')}` : 'Aguardando...'}
          </div>
        </div>
      )}
    </>

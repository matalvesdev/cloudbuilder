...
import {
  CostEstimationBar,
  getResourcePrice,
} from "./components/CostEstimationBar";
...
export function DesignModule() {
  // Enable keyboard shortcuts (Delete, Escape, Ctrl+Z/Y/D)
  useCanvasKeyboard(true);
  ...
  const totalEstimatedCost = useMemo(() => {
    return nodes.reduce(
      (sum, n) => sum + getResourcePrice(n.data?.resourceType || ""),
      0,
    );
  }, [nodes]);
  ...
  const handleSave = useCallback(() => {
    ...
    localStorage.setItem(
      `cloudbuilder-canvas-v${newVersion}`,
    ...
    try {
      ...
      const updated = [
        entry,
        ...history.filter((h: any) => h.version !== newVersion),
      ].slice(0, 20);
    ...
    generateDocFromCanvas(
      canvasId,
      canvasName,
      `Design com ${state.nodes.length} recursos e ${state.edges.length} conexões`,
    )
      .then(() => {
        /* silent */
      })
    ...
    if (tenantId && userId) {
      saveToBackend(tenantId, userId)
        ...
        .catch((err) => {
          ...
          showWarning(
            "Design salvo localmente, mas falha ao salvar no servidor",
          );
        });
    } else {
  ...
  const handleExportImage = useCallback(async () => {
    ...
    try {
      ...
      clone
        .querySelectorAll(".react-flow__minimap, .react-flow__controls")
        .forEach((n) => n.remove());
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const isTerraform =
        file.name.endsWith(".tf") || file.name.endsWith(".tf.json");
  ...
  const handleValidate = useCallback(() => {
    const { nodes, edges, setNodesValidationStatus } =
  ...
  const { alignNodes, distributeNodes, autoLayout, duplicateSelected } =
    useCanvasStore();
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 relative">
      ...
      <input
        ref={fileInputRef}
        type="file"
        accept=".cloudbuilder.json,.json,.tf,.tf.json"
        className="hidden"
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-red-50 border border-red-200 rounded-xl shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-red-700 truncate">
                    Custo estimado de ~${totalEstimatedCost.toFixed(0)}/mês
                    excede o limite de ${BUDGET_THRESHOLD}/mês
                  </p>
                </div>
                <button
              ...
              <TooltipProvider>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Toggle
                      size="sm"
                      pressed={showPalette}
                      onPressedChange={() => setShowPalette(!showPalette)}
                      className="w-[30px] h-[30px] rounded-full p-0 data-[state=on]:bg-brand-navy data-[state=on]:text-brand-lime hover:bg-ice-blue hover:text-brand-navy shrink-0"
                    >
                ...
                <DropdownMenu>
                  ...
                  >
                    ...
                    <DropdownMenuItem
                      onSelect={() => setShowCodePreview(!showCodePreview)}
                    >
                    ...
                    <DropdownMenuItem
                      onSelect={() => {
                        ...
                        setShowCollaboration(false);
                        if (showAIChat) setShowProperties(true);
                      }}
                    >
                    ...
                    <DropdownMenuItem
                      onSelect={() => {
                        ...
                        setShowAIChat(false);
                        if (showCollaboration) setShowProperties(true);
                      }}
                    >
                    ...
                    <DropdownMenuItem
                      onSelect={() => {
                        setShowCostEstimation(!showCostEstimation);
                        setShowAIChat(false);
                        setShowCollaboration(false);
                ...
                <DropdownMenu>
                  ...
                  >
                    <DropdownMenuItem
                      onSelect={() => setShowTerraformImport(true)}
                    >
                      <FileCode className="w-4 h-4 text-slate-500" />
                    ...
                    >
                      <LayoutGrid className="w-4 h-4 text-slate-500" />
                      <span>
                        Snap ao grid {snapEnabled ? "(ativo)" : "(inativo)"}
                      </span>
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
                  ...
                  (() => {
                    ...
                    return (
                      <div className="flex items-center gap-1.5 px-2 py-1 bg-brand-lime/10 rounded-full shrink-0">
                        ...
                        <span className="text-[10px] font-semibold text-brand-navy whitespace-nowrap">
                ...
                {nodes.length > 0 && (
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom">
                      {isOverBudget
                        ? "Custo excede limite — clique para detalhes"
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Toggle
                      size="sm"
                      pressed={showMetrics}
                      onPressedChange={() => setShowMetrics(!showMetrics)}
                      className="w-[30px] h-[30px] rounded-full p-0 data-[state=on]:bg-brand-navy data-[state=on]:text-brand-lime hover:bg-ice-blue hover:text-brand-navy shrink-0"
                    >
                  ...
                  <TooltipContent side="bottom">
                    {showMetrics ? "Desativar métricas" : "Métricas ao vivo"}
                  </TooltipContent>
                </Tooltip>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Toggle
                      size="sm"
                      pressed={showRepoBrowser}
                      onPressedChange={() => {
                        if (!githubToken) setShowGitHubConnect(true);
                        else setShowRepoBrowser(!showRepoBrowser);
                      }}
                      className="w-[30px] h-[30px] rounded-full p-0 data-[state=on]:bg-slate-900 data-[state=on]:text-white hover:bg-ice-blue hover:text-brand-navy shrink-0"
                    >
                      <Github className="w-4 h-4" />
                    </Toggle>
                  </TooltipTrigger>
                  <TooltipContent side="bottom">
                    {githubToken ? "GitHub conectado" : "Conectar GitHub"}
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
            ...
            {[
              {
                ...
                toggle: () => {
                  ...
                  setShowCanvasDocs(false);
                  setShowCanvasLogs(false);
                  setShowCanvasEvents(false);
                  setShowCanvasConsole(false);
                },
              },
              {
                key: "docs",
                ...
                show: showCanvasDocs,
                toggle: () => {
                  setShowCanvasDocs(!showCanvasDocs);
              ...
              },
              {
                ...
                show: showCanvasLogs,
                toggle: () => {
                  setShowCanvasLogs(!showCanvasLogs);
                  setShowCodePreview(false);
                  setShowCanvasDocs(false);
                  setShowCanvasEvents(false);
                  setShowCanvasConsole(false);
              ...
              },
              {
                key: "events",
                label: "Eventos",
                show: showCanvasEvents,
                toggle: () => {
                  setShowCanvasEvents(!showCanvasEvents);
                  setShowCodePreview(false);
                  setShowCanvasDocs(false);
                  setShowCanvasLogs(false);
                  setShowCanvasConsole(false);
              ...
              },
              {
                key: "console",
                label: "Console",
                show: showCanvasConsole,
                toggle: () => {
                  setShowCanvasConsole(!showCanvasConsole);
                  setShowCodePreview(false);
                  setShowCanvasDocs(false);
                  setShowCanvasLogs(false);
                  setShowCanvasEvents(false);

...
import { dashboardApi, type CostOverview, type ObserveDashboard, type CanvasSummary } from '../dashboardApi'
import { mockFetch, mockFetchNetworkError, restoreFetch } from '@/test/mockFetch'
...
describe('dashboardApi', () => {
  ...
  describe('getCostOverview', () => {
    ...
    it('retorna null em caso de erro 500', async () => {
      mockFetch({ status: 500, body: { status: 500, message: 'Internal error', timestamp: new Date().toISOString() } })
      const result = await dashboardApi.getCostOverview(envId)
      expect(result).toBeNull()
    })
  })
  ...
  describe('getCanvases', () => {
    it('retorna lista de canvases', async () => {
      const expected: CanvasSummary[] = [
        { id: '1', name: 'VPC', nodeCount: 5, edgeCount: 3, updatedAt: '2026-06-14T00:00:00Z' },
      ]

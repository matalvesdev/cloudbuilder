...
import { mockFetch, mockFetchNoContent, mockFetchNetworkError, restoreFetch } from '@/test/mockFetch'
...
describe('HttpClient', () => {
  ...
  describe('GET', () => {
    it('faz GET e retorna JSON', async () => {
      ...
      const result = await client.get<{ id: string; name: string }>('/canvases')
    ...
    it('trata 401 limpando tokens', async () => {
      ...
      mockFetch({ status: 401, body: { status: 401, message: 'Unauthorized', timestamp: new Date().toISOString() } })
    ...
    it('trata erro de rede', async () => {
      ...
      await expect(client.get('/canvases')).rejects.toMatchObject({ status: 0 })
    })

    it('trata erro 400 com mensagem', async () => {
      mockFetch({ status: 400, body: { status: 400, message: 'Bad request', timestamp: new Date().toISOString() } })
      await expect(client.get('/canvases')).rejects.toMatchObject({ status: 400, message: 'Bad request' })
    })
  ...
  describe('POST', () => {
    it('faz POST com body JSON', async () => {
      ...
      globalThis.fetch = async (_url: string | URL | Request, init?: RequestInit) => {
        ...
        return new Response(JSON.stringify({ id: 'new-1' }), { status: 200, headers: { 'Content-Type': 'application/json' } })
      }
      const result = await client.post<{ id: string }>('/canvases', { name: 'Meu Design' })
  ...
  describe('PUT', () => {
    it('faz PUT com body JSON', async () => {
      ...
      const result = await client.put<{ id: string; name: string }>('/canvases/123', { name: 'Atualizado' })

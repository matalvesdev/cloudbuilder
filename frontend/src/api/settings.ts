...
export const settingsApi = {
  ...
  createToken: (name: string, scopes?: string) => api.post<CreateTokenResponse>('/api-tokens', { name, scopes: scopes || 'read,write' }),
  revokeToken: (id: string) => api.delete<void>(`/api-tokens/${id}`),

  // SSH Keys
  listSshKeys: () => api.get<SshKeyDTO[]>('/ssh-keys'),
  addSshKey: (name: string, publicKey: string) => api.post<SshKeyDTO>('/ssh-keys', { name, publicKey }),

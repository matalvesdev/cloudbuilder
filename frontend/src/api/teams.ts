...
export function searchTeams(orgId: string, q: string): Promise<Team[]> {
  return api.get(
    `/organizations/${orgId}/teams/search?q=${encodeURIComponent(q)}`,
  );
}
...
export function searchSquads(
  ...
  return api.get(
    `/organizations/${orgId}/squads/search?workspaceId=${encodeURIComponent(workspaceId)}&q=${encodeURIComponent(q)}`,
  );
}

export function listSquadsByWorkspace(
  orgId: string,
  workspaceId: string,
): Promise<Squad[]> {
...
export function listMembersByTeam(
  orgId: string,
  teamId: string,
...
export function assignToTeam(
  ...
  return api.post(`/organizations/${orgId}/members/${membershipId}/team`, {
    teamId,
  });
}

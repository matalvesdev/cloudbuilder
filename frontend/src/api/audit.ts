...
export const auditApi = {
  ...
  getEvent: (id: string) => api.get<AuditEvent>(`/audit/events/${id}`),
  ...
  exportJson: (tenantId: string, params?: AuditQueryParams) => {
    ...
    return api.get<AuditEvent[]>(
      `/audit/export/${tenantId}/json${qs ? "?" + qs : ""}`,
    );
  },
  ...
  getComplianceEvaluations: (tenantId: string) =>
    api.get<ComplianceEvaluation[]>(
      `/audit/compliance/${tenantId}/evaluations`,
    ),

  listComplianceRules: (tenantId?: string) =>
    api.get<ComplianceRule[]>(
      tenantId
        ? `/audit/compliance/rules/${tenantId}`

...
export interface ReplaceCanvasContentRequest {
...
export function listCanvases(
  page = 0,
  size = 20,
): Promise<PaginatedResponse<CanvasDTO>> {
...
export function replaceCanvasContent(
  id: string,
  request: ReplaceCanvasContentRequest,
): Promise<CanvasDTO> {
  return api.put(`${BASE}/${id}/content`, request);
}
...
export function createVersion(
  canvasId: string,
  changeDescription: string = "Nova versão",

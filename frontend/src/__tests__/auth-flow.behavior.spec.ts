...
describe('Auth: Login Flow', () => {
  ...
  it('Given no user is logged in, When login fails with wrong password, Then error is displayed and user remains unauthenticated', async () => {
    ...
    // When
    const authApi = await import('@/api/auth')
    vi.mocked(authApi.login).mockRejectedValueOnce(new Error('Credenciais inválidas'))
  ...
  it('Given login is in progress, Then isLoading is true until completion', async () => {
    ...
    vi.mocked(authApi.login).mockImplementation(
      () => new Promise((resolve, reject) => { resolveLogin = resolve }),
    )
    ...
    // Resolve
    resolveLogin({ userId: 'u1', name: 'U', email: 'u@t.co', roles: ['V'], token: 't' })
    await loginPromise
...
describe('Auth: Register Flow', () => {
  it('Given new user, When registration succeeds, Then user is auto-authenticated', async () => {
    ...
    await useAuthStore.getState().register(
      'Marina Costa', 'marina@startup.io', 'strongPass123!', 'StartupIO', 'startupio',
    )
  ...
  it('Given new user, When registration fails (duplicate email), Then error is shown', async () => {
    ...
    // When
    const authApi = await import('@/api/auth')
    vi.mocked(authApi.register).mockRejectedValueOnce(new Error('Email já cadastrado'))

    try {
      await useAuthStore.getState().register(
        'Duplicate User', 'existing@test.com', 'pass123',
      )
    } catch {}
...
describe('Auth: Role-based Access Matrix', () => {
  it.each([
    ...
    (role, canCreate, canEdit, canDelete) => {
      ...
      expect(roles.includes('ADMIN') || roles.includes('EDITOR')).toBe(canCreate)

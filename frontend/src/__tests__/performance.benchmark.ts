...
describe("CanvasStore Performance", () => {
  bench("addNode (single)", () => {
    ...
    store.addNode(makeComponentData("Test Node"), { x: Math.random() * 1000, y: Math.random() * 1000 });
  }, { iterations: 1000 });

  bench("addNode (batch 100)", () => {
    useCanvasStore.setState({ nodes: [], undoStack: [] })
    const store = useCanvasStore.getState();
    for (let i = 0; i < 100; i++) {
  ...
  bench("node lookup in 100-node graph", () => {
    useCanvasStore.setState({ nodes: [], edges: [], undoStack: [] })
    const store = useCanvasStore.getState();
    for (let i = 0; i < 100; i++) {
...
describe("CostStore Performance", () => {
  bench("fetchCostData (mock)", () => {
    const store = useCostStore.getState();
    store.fetchCostData();
  }, { iterations: 100 });

  bench("anomaly detection (compute)", () => {

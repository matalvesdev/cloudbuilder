...
class EventBus {
  ...
  off<K extends keyof EventMap>(
    ...
    this.handlers
      .get(event as string)
      ?.delete(cb as (...args: unknown[]) => void);
  }
...
class NativeEventBus {
  ...
  private createWebSocket(userInfo: {
    id: string;
    name: string;
    avatar: string;
  }): void {
    ...
    const url = `${this.serverUrl}/ws/${encodeURIComponent(this.roomId)}?token=${encodeURIComponent(token)}`;
  ...
  private scheduleReconnect(userInfo: {
    id: string;
    name: string;
    avatar: string;
  }): void {

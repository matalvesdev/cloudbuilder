...
class ProvisionApiService {
  ...
  async listResources(environmentId: string): Promise<ManagedResource[]> {
    return api.get<ManagedResource[]>(
  ...
  async syncResources(
    ...
    return api.post<ManagedResource[]>(`/environments/${environmentId}/sync`, {
      stateJson,
    });
  }
  ...
  async getDriftHistory(environmentId: string): Promise<DriftReport[]> {
    return api.get<DriftReport[]>(
      `/environments/${environmentId}/drift/history`,
    );
  }
  ...
  async extendEphemeral(
    id: string,
    extraHours: number,
  ): Promise<EphemeralEnvironment> {
    return api.post<EphemeralEnvironment>(`/ephemeral/${id}/extend`, {
      extraHours,
    });
  }
  ...
  async getRegionDeployments(
    environmentId: string,
  ): Promise<RegionDeployment[]> {
    return api.get<RegionDeployment[]>(
      `/dr/environments/${environmentId}/regions`,
    );
  }

  async addRegionDeployment(
    environmentId: string,
    region: string,
    primary: boolean,
    priority: number,
  ): Promise<RegionDeployment> {
    return api.post<RegionDeployment>(
      `/dr/environments/${environmentId}/regions`,
      { region, primary, priority },
    );
  }
  ...
  async importTerraform(
    content: string,
  ): Promise<{ nodes: unknown[]; connections: unknown[]; warnings: string[] }> {
    return api.post("/import/terraform", { content });
  }

  async importState(content: string): Promise<{
    resources: unknown[];
    connections: unknown[];
    warnings: string[];
    resourceCount: number;

        ...
        <nav className="flex items-center gap-0.5 shrink-0">
          {navGroups.map((group) => {
            const visibleItems = group.items.filter((item) => {
              ...
              if (item.id === "finops")
                return useUiStore.getState().isEnabled("module.cost");
              if (item.id === "platform")
                return useUiStore.getState().isEnabled("module.platform");
              if (item.id === "ai")
                return useUiStore.getState().isEnabled("module.aiops");
              if (item.id === "security")
                return useUiStore.getState().isEnabled("module.audit");
              return true;
            });
              ...
              {searchResults.length === 0 ? (
                ...
                searchResults.map((r) => (
                  <button
                    key={r.id}
                    onClick={() => {
                      onModuleChange(r.id as ModuleId);
                      setQuery("");
                      setShowSearch(false);
                    }}
                    ...
                    {user?.roles && user.roles.length > 0 && (
                      <div className="flex items-center gap-1.5 mt-3">
                        {user.roles.map((role) => (
                          <span
                            key={role}
                            className="text-[9px] px-1.5 py-0.5 rounded-full bg-white/15 font-medium"
                          >
                            {role === "admin"
                              ? "Admin"
                              : role === "editor"

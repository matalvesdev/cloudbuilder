...
export function OnboardingLayout({ view, onViewChange }: OnboardingLayoutProps) {
  ...
  if (view === 'welcome') {
    return (
      <OnboardingWelcome
        ...
        onStartSetup={() => { setStage('gateway-setup'); onViewChange('gateway') }}
        onSkip={() => { skipOnboarding(); onViewChange('done') }}
      />
    )
  }
  ...
  if (view === 'tour') {
    return (
      <OnboardingTour
        onComplete={() => { markTourCompleted(); onViewChange('done') }}
